...
Additionally, the ORIGINAL fixture data in `tests/unit/fixtures_test.go` — which predates the VPC/SG/NodeGroup work — also contains real dev-account data (real instance IDs, bucket names, IP addresses, endpoints, secret names). This file was never sanitized either.
...
|------|---------------------|----------|
| `tests/testdata/fixtures_vpc.go` | Account ID (999000111222), 2 real VPC IDs, DHCP option ID, CIDR 10.10.0.0/16, CIDR assoc IDs, tag "vpc-dev" | CRITICAL |
| `tests/testdata/fixtures_sg.go` | Account ID (999000111222) x50+, 21 real SG IDs, 21 real SG ARNs, real VPC IDs, real IP CIDRs (10.10.x.x), real org resource names (emo-ai-dev, multimedia-mod-dev, gh-ci-ubuntu), SSM config paths, karpenter tags | CRITICAL |
| `tests/testdata/fixtures_nodegroups.go` | Account ID (999000111222), cluster name eks-eu-west-2-dev, 3 real node group ARNs, 3 real IAM role ARNs, 3 real subnet IDs, 3 real launch template IDs, real ASG names | CRITICAL |

### CRITICAL — Test files asserting on real AWS values

| File | Sensitive Data Types | Severity |
|------|---------------------|----------|
| `tests/unit/aws_vpc_test.go` | Asserts on real VPC IDs (vpc-05a7ebc57fd26ae33, vpc-09f63b3acf1049f1d), account ID 999000111222, real DHCP option ID, real CIDR 10.10.0.0/16 | CRITICAL |
...
| `tests/unit/fixtures_test.go` | Real S3 bucket names (cdn-cloudfront-logs.dev-acct.cloud, dev-account-fileshare, etc.), real EC2 instance IDs (6x), real IPs (10.10.x.x, 35.176.64.234, 13.43.142.76), real RDS/DocDB endpoints with cluster hash (cziaoicgy5um), real EKS endpoint hash (9F74DCF3EA0BB96E...), real secret names | CRITICAL |
...
| `tests/unit/qa_s3_test.go` | Real bucket names (auth-service-dev-state, dev-account-fileshare) |
...
| `tests/unit/qa_detail_paths_test.go` | Comment referencing "dev-account AWS account" |
| `tests/unit/qa_profile_switch_test.go` | Uses "dev-account" and "dev-acct-prod" as profile names (lower risk — profile names are not infrastructure data, but do reveal org naming) |
...
| `cdn-cloudfront-logs.dev-acct.cloud` | `cdn-logs.example.com` |
| `cdn-test-website.dev-acct.cloud` | `cdn-website.example.com` |
| `dev-account-fileshare` | `dev-fileshare` |
| `dev-account-loki-chunks` | `dev-loki-chunks` |
...
| Real Value | Dummy Replacement | Notes |
|-----------|-------------------|-------|
| `dev-account` | Keep as-is in agent config | Only sanitize in test assertions and comments |
| `dev-acct-prod` | `staging-profile` | In test assertions |
...
- Replace account ID `999000111222` -> `123456789012`
...
- Update comment header: remove "Account: 999000111222", change to "Account: 123456789012 (sanitized)"
- Remove "dev-account" from source comment

#### 5.2 `tests/testdata/fixtures_sg.go`
- Replace ALL instances of `999000111222` -> `123456789012`
...
- Update test function name/comment: remove "dev-account" reference
...
- Update comments: remove "dev-account" references
...
- Replace bucket names (auth-service-dev-state, dev-acct patterns)
...
- Remove "dev-account" from comment on line 17
...
| Organization-specific names (dev-acct, emo-ai, etc.) | Generic equivalents |
...
Consider adding a CI check or pre-commit hook that scans for the known real account ID (`999000111222`) in any `.go`, `.md`, or `.yaml` file. If found, fail the check.

### Rule 4: Agent review gate
The a9s-tui-reviewer agent must check for real AWS data patterns during code review. Any PR containing real VPC IDs, SG IDs, account IDs, or org-specific names (dev-acct, emo-ai, etc.) must be rejected.
...
7. **Verify** — grep the entire repo for `999000111222` and other sentinel values to confirm zero leaks
