...
func (m DetailModel) Update(msg tea.Msg) (DetailModel, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		// Delegate to search when in input mode (search captures all keys).
		...
		// Handle remaining key bindings via key.Matches so they work for
		// both KeyPressMsg (integration tests) and KeyMsg (runtime) paths.
		switch {
		...
		case key.Matches(msg, m.keys.Copy):
			// Left column: copy the field value at cursor (right col handled above).
			if m.fieldList != nil && m.fieldCursor >= 0 && m.fieldCursor < len(m.fieldList) {
				item := m.fieldList[m.fieldCursor]
				val := item.Value
				if val == "" {
