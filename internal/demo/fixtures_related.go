...
func init() {
	...
	// ct-events intentionally has no RegisterRelatedDemo override: all 17 checkers
	// are pure field-readers (no AWS calls) and produce correct results when given
	// a cache populated from demo fixtures. The fall-through in app_related.go
	// invokes the real checkers directly for every ct-events def.
