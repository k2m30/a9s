...
// Fetcher, Wave2, Related, and Navigable are intentionally nil/empty here: those
// fields require concrete internal/aws types, which must NOT be imported from
// internal/catalog (boundary rule in types.go). The legacy init() registrations
// in internal/aws continue to populate the legacy registries for those fields
// until PR-04n restructures the import graph.
