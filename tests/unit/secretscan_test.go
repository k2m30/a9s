...
func TestScanText_HighEntropyKeywordValue_BothKindsSameLine(t *testing.T) {
	text := "SECRET_KEY=OhbVrpoiVgRV5IfLBcbfnoGMbJmTPS\n"
	hits := secretscan.ScanText(text)
	wantKeyword := secretscan.Hit{Kind: "keyword", Where: "line 1"}
	wantEntropy := secretscan.Hit{Kind: "high-entropy", Where: "line 1"}
	hasKeyword, hasEntropy := false, false
	for _, h := range hits {
		if h == wantKeyword {
			hasKeyword = true
		}
	...
	if !hasKeyword || !hasEntropy {
		t.Errorf("ScanText(%q) = %v, want both %v and %v", text, hits, wantKeyword, wantEntropy)
	}
}
