...
// TestRelated_Athena_Stubs verifies all 2 related defs are registered as stubs (nil checkers).
func TestRelated_Athena_Stubs(t *testing.T) {
	...
	for target, wantDisplay := range expected {
		...
		for _, def := range defs {
			if def.TargetType == target {
				found = true
				if def.Checker != nil {
					t.Errorf("athena %q: Checker should be nil (stub)", target)
				}
