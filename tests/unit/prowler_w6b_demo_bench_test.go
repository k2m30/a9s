...
// The per-type tests prove a fetcher or enricher emits the right finding for
// the input it is handed. These prove the demo account contains such an
// input: every CI/CD, data and container posture code is carried by exactly
// one row after the real fetcher and enricher path, and no row that was
// meant to be healthy is quietly tripping the same condition.
...
// The row name comes from the fixture constant rather than a literal, so
// renaming the demo resource moves this pin with it.
...
// w6bBatchTypes are the types the sweeps below cover.
var w6bBatchTypes = map[string]bool{
	"cfn": true, "cb": true, "ecr": true, "codeartifact": true, "glue": true, "eks": true,
}

// w6bMergedDemoRows folds the demo rows of every w6bBatchTypes type the way
// the app folds them, so a gate reading them sees the same detail block a
// user does.
...
// Exactly one matters in both directions. Zero means the fixture never
// reaches the condition and the signal is invisible in the demo. More than
// one means a row that was supposed to be healthy is not.
...
// TestW6BDetailAttentionNeverRepeatsItself pins that a supporting row adds
// something its OWN finding's phrase does not already say. The row and the
// phrase render one line apart, so a row that normalises to its phrase is the
// detail block printing one fact twice.
...
// w6bRowlessCodes are the findings whose phrase is the whole fact. A
// supporting row would restate it — "Termination protection: off" under
// "termination protection off" — and paraphrases ("Tag mutability: mutable"
// under "tags are mutable") slip past the normalised comparison above, which
// is why this list is explicit rather than derived.
//
...
// disabled log types, the argument name, the key that held the credential.
...
// rendered row of the w6bBatchTypes. The SDK's enum casing ("PUBLIC_READ",
...
// Every finding the w6bBatchTypes emit on a demo row carries an operator
// sentence. A registered code with an empty Detail renders a blank line where
// the explanation belongs, and no per-type test can see that for the demo
// path.
...
// Every demo cluster must report a version the registry publishes: a cluster
// on a minor the registry has never heard of is classified "unknown" and
// silently drops out of the support-status check, which looks exactly like a
// healthy cluster.
