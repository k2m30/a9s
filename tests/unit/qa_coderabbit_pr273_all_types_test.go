...
//   * hasEnricher        — true if awsclient.Wave2EnricherFor(shortName)
//                          returns ok=true; Wave 2 enrichment is an additional
...
var typeContracts = []typeContract{
	...
	{shortName: "dbi-snap", apiDoc: "https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_DescribeDBSnapshots.html", statusField: "status", healthyStatuses: []string{"available"}, warningStatuses: []string{"creating", "copying"}, brokenStatuses: []string{"failed"}, reasoning: "RDS snapshot Status per DescribeDBSnapshots — 'failed' surfaces as broken."},
	{shortName: "redshift", apiDoc: "https://docs.aws.amazon.com/redshift/latest/APIReference/API_DescribeClusters.html", statusField: "cluster_status", healthyStatuses: []string{"available"}, warningStatuses: []string{"creating", "modifying", "deleting", "resizing", "renaming", "rebooting"}, brokenStatuses: []string{"incompatible-hsm", "incompatible-network", "incompatible-parameters", "incompatible-restore", "hardware-failure", "storage-full"}, reasoning: "Redshift ClusterStatus (raw AWS enum) lives in Fields[\"cluster_status\"] after the §4 phrase migration (2026-04-24). Fields[\"status\"] carries the derived phrase; Color reads the raw enum. incompatible-*/hardware-failure/storage-full surface as broken."},
	...
	{shortName: "ses", apiDoc: "https://docs.aws.amazon.com/ses/latest/APIReference-V2/API_ListEmailIdentities.html", statusField: "status", healthyStatuses: []string{""}, warningStatuses: []string{"pending verification", "sending disabled"}, brokenStatuses: []string{"verification failed", "verify: temp failure", "verification not started"}, reasoning: "SES Color reads Fields[\"status\"] (derived phrase from sesTopPhrase) not the raw verification_status enum. Healthy = empty phrase; Warning = pending/sending-disabled; Broken = failed/temp-failure/not-started."},
	...
	{shortName: "dbc", apiDoc: "https://docs.aws.amazon.com/documentdb/latest/developerguide/API_DescribeDBClusters.html", statusField: "status", healthyStatuses: []string{"", "maintenance overdue"}, warningStatuses: []string{"creating: in progress", "modifying: in progress", "backing-up: in progress", "upgrading: in progress", "delete-protection off", "not encrypted at rest", "no automated backups", "delete-protection off (+2)"}, brokenStatuses: []string{"failed: cluster operation", "encryption key unreachable", "parameter group incompatible", "no writer: reads only"}, reasoning: "DocDB dbc now stores the §4 Status PHRASE (not the raw AWS keyword) in Fields[\"status\"]. Healthy renders blank; Wave-2 'maintenance overdue' on a healthy row stays green so the `!` glyph can render. Per docs/resources/dbc.md §4."},
	...
	{shortName: "mwaa", apiDoc: "https://docs.aws.amazon.com/mwaa/latest/API/API_GetEnvironment.html", statusField: "", reasoning: "MWAA ListEnvironments returns names only (no Wave 1 status field); every signal is emitted fetcher-side from GetEnvironment (Source \"wave1\", no separate Wave2 enricher — docs/resources/mwaa-impl-plan.md §0). colorMWAA is colorAnyFindingOrHealthy (catalog_color_helpers.go) — it colors from ANY finding, with no raw-status whitelist. Fields[\"status\"] carries a §4 phrase, not a bare AWS keyword, so this row sets statusField=\"\" and the list-level color-classification subtest is skipped via the statusField==\"\" branch. Full status→bucket coverage lives in aws_mwaa_test.go (StatusMappingAllTwelveValues), which exercises colorMWAA's real Findings via the actual fetcher output."},
	...
	{shortName: "ddb", apiDoc: "https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_DescribeTable.html", statusField: "status", healthyStatuses: []string{"", "PITR off"}, warningStatuses: []string{"creating", "updating", "deleting", "archiving"}, brokenStatuses: []string{"kms key inaccessible", "archived: kms key lost"}, hasEnricher: true, reasoning: "DynamoDB Tables — post-spec-rewrite (2026-04-23): Fields[\"status\"] carries §4 PHRASES (lowercase), not raw AWS enums. Healthy = blank silence; Wave-2 'PITR off' on Healthy row stays green (~ glyph does the signaling); transitional phrases = Warning; KMS-related failure phrases = Broken. Spec: docs/resources/ddb.md §4."},
	...
	{shortName: "ecs-task", apiDoc: "https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeTasks.html", statusField: "status", healthyStatuses: []string{"RUNNING"}, warningStatuses: []string{"PROVISIONING", "PENDING", "ACTIVATING", "DEACTIVATING", "STOPPING", "DEPROVISIONING"}, brokenStatuses: nil, reasoning: "ECS Task LastStatus per DescribeTasks. STOPPED is terminal — Dim by default; Broken only when stop_code != UserInitiated (e.g. TaskFailedToStart, EssentialContainerExited). See qa_ecs_task_color_test.go for full coverage including stop_code and health_status overrides."},
	{shortName: "efs", apiDoc: "https://docs.aws.amazon.com/efs/latest/ug/API_DescribeFileSystems.html", statusField: "status", healthyStatuses: []string{"available", ""}, warningStatuses: []string{"creating", "updating", "deleting"}, brokenStatuses: []string{"error", "no mount targets"}, reasoning: "EFS FileSystemDescription.LifeCycleState: creating | available | updating | deleting | deleted | error."},
	...
	{shortName: "redis", apiDoc: "https://docs.aws.amazon.com/AmazonElastiCache/latest/APIReference/API_DescribeReplicationGroups.html", statusField: "status", healthyStatuses: []string{""}, warningStatuses: []string{"creating — new group", "modifying — config change", "snapshotting — backup running", "deleting — teardown", "multi-AZ without auto-failover"}, brokenStatuses: []string{"create failed — see events"}, reasoning: "ElastiCache ReplicationGroup — post-spec-rewrite: Fields[\"status\"] = §4 phrase (Healthy silence = empty; Warning = bare phrase; Broken = 'create failed — see events'). Spec: docs/resources/redis.md §4."},
	...
	{shortName: "transfer", apiDoc: "https://docs.aws.amazon.com/transfer/latest/APIReference/API_DescribeServer.html", statusField: "status", healthyStatuses: []string{"ONLINE"}, warningStatuses: []string{"offline: not accepting transfers", "starting", "stopping", "stop failed", "legacy security policy", "no activity logging"}, brokenStatuses: []string{"start failed"}, reasoning: "colorTransfer (core/aws/catalog_networking.go) is colorFromAnyFinding-only (docs/resources/transfer.md §4) — every off-ONLINE DescribeServer State and config-health signal is emitted fetcher-side as a Finding (computeTransferFindings in transfer.go): offline/starting/stopping/stop_failed/legacy_policy/no_logging carry SevWarn, start_failed carries SevBroken. This table's synthetic-Finding injection pins that Severity contract per code."},
	...
	{shortName: "vpc-peer", apiDoc: "https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_VpcPeeringConnection.html", statusField: "status", healthyStatuses: []string{""}, warningStatuses: []string{"provisioning", "initiating", "pending acceptance: expires in 3d", "expired: never accepted", "deleting", "CIDR overlap with peer", "no local route to peer", "route to peer blackholed"}, brokenStatuses: []string{"rejected", "failed"}, hasEnricher: true, reasoning: "colorVpcPeer (core/aws/catalog_networking.go) is colorFromAnyFinding-only (docs/resources/vpc-peer.md §4) — every Wave 1 state/CIDR-overlap signal is fetcher-written (Source \"wave1\") and both Wave 2 cache-scan signals (EnrichVpcPeerRoutes) mirror the lt deprecated-AMI '~' treatment; there is no raw Status.Code fallback. 'deleted' (SevDim) is real per DescribeVpcPeeringConnections but not exercised by this table — mirrors the nat/tgw/vpce precedent of leaving Dim untested here."},
