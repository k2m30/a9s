...
var typeContracts = []typeContract{
	...
	{shortName: "mwaa", apiDoc: "https://docs.aws.amazon.com/mwaa/latest/API/API_GetEnvironment.html", statusField: "", reasoning: "MWAA ListEnvironments returns names only (no Wave 1 status field); every signal is emitted fetcher-side from GetEnvironment (Source \"wave1\", no separate Wave2 enricher — docs/resources/mwaa-impl-plan.md §0). colorMWAA gates on mwaaStateCodes, a whitelist of the real FindingCodes the fetcher assigns per Status/LastUpdate/WebserverAccessMode — this table's synthetic-Finding injection (an arbitrary test Code) can never match that whitelist, so list-level classification is skipped here. Full status→bucket coverage lives in aws_mwaa_test.go (StatusMappingAllTwelveValues, StatusPhraseAndSeverity), which exercises colorMWAA's real Codes via the actual fetcher output."},
