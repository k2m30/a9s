...
// S3 server-access logs are delivered to
// another S3 bucket (GetBucketLogging.LoggingEnabled.TargetBucket), never to
// CloudWatch Logs, so the access-log pivot targets `s3` with a DisplayName
// that names the destination bucket. A `logs` target would land on an empty
// list: no log group matches a bucket name (docs/resources/s3.md).
...
func TestS3_AccessLogPivot_TargetsS3BucketNotCloudWatch(t *testing.T) {
	...
	// Any other target leads to an empty list: the destination is another S3
	// bucket.
	...
	// A TargetType="logs" access-log registration would double-list the pivot
	// with a dead navigation path.
