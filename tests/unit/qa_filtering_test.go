...
func TestQA_Filter_11_02_MainMenu_FilterEC2ShowsOnlyEC2(t *testing.T) {
	...
	// Frame title should show resource-types(1/70) — +1 for the permanent
	// synthetic "costs" entry alongside resource.AllResourceTypes().
	if !strings.Contains(plain, "resource-types(1/70)") {
		t.Errorf("frame title should show resource-types(1/70), got: %s", plain)
	}
}
...
func TestQA_Filter_11_04_MainMenu_FilterNoMatch(t *testing.T) {
	...
	// Frame title should show resource-types(0/70)
	if !strings.Contains(plain, "resource-types(0/70)") {
		t.Errorf("frame title should show resource-types(0/70), got: %s", plain)
	}
}
...
func TestQA_Filter_11_06_MainMenu_Backspace(t *testing.T) {
	...
	plain := stripANSI(rootViewContent(m))
	if !strings.Contains(plain, "resource-types(1/70)") {
		t.Fatalf("precondition: filter 'ec2' should show 1/70, got: %s", plain)
	}
	...
	m, _ = rootApplyMsg(m, tea.KeyPressMsg{Code: tea.KeyBackspace})
	plain = stripANSI(rootViewContent(m))
	// All 70 items should be back (69 resource types + the synthetic costs entry)
	if !strings.Contains(plain, "resource-types(70)") {
		t.Errorf("after clearing filter, frame should show resource-types(70), got: %s", plain)
	}
}
...
func TestQA_Filter_11_07_MainMenu_FrameTitleCount(t *testing.T) {
	...
	// Must have format resource-types(N/70) where N < 10
	if !strings.Contains(plain, "/70)") {
		t.Errorf("filtered frame title should contain /70) showing filtered count, got: %s", plain)
	}
}
...
func TestQA_Filter_11_08_MainMenu_EscClearsFilter(t *testing.T) {
	...
	if !strings.Contains(plain, "resource-types(70)") {
		t.Errorf("after Esc, frame title should be resource-types(70), got: %s", plain)
	}
...
func TestQA_Filter_11_24c_EscClearsConfirmedFilterOnMainMenu(t *testing.T) {
	...
	if !strings.Contains(plain, "resource-types(70)") {
		t.Errorf("Esc should clear confirmed filter, showing all 70 types, got: %s", plain)
	}
}
