// fakes_docdb_test.go is the single source for DocumentDB SDK-interface
...
// fakeDocDBDescribeDBClusters implements awsclient.DocDBDescribeDBClustersAPI.
//
...
type fakeDocDBDescribeDBClusters struct {
	Err      error
...
func (f *fakeDocDBDescribeDBClusters) DescribeDBClusters(
	_ context.Context,
...
// fakeDocDBDescribeDBClusterSnapshots implements
// awsclient.DocDBDescribeDBClusterSnapshotsAPI.
//
// Configure at most one response mode:
//   - Err: always return this error
//   - Output: always return this single output (no pagination)
...
// Calls, Inputs and LastInput are always recorded so tests can assert on
// call count and forwarded pagination tokens regardless of response mode.
type fakeDocDBDescribeDBClusterSnapshots struct {
	Err      error
...
func (f *fakeDocDBDescribeDBClusterSnapshots) DescribeDBClusterSnapshots(
	_ context.Context,
