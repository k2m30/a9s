...
// TestRelated_TGW_CfnStub verifies that the cfn related def is registered but
// has a nil Checker (stub, not yet implemented).
func TestRelated_TGW_CfnStub(t *testing.T) {
	defs := resource.GetRelated("tgw")
	for _, def := range defs {
		if def.TargetType == "cfn" {
			if def.Checker != nil {
				t.Error("tgw cfn: expected nil Checker (stub)")
			}
			return
		}
	}
	t.Error("tgw cfn related def not found")
}
