...
// Path preservation through the
// fieldItemToDomainItem → domainItemToFieldItem round-trip.
//
// fieldItemToDomainItem (in generic.go) copies fi.Path directly into
// domain.Item.Path, and domainItemToFieldItem (in detail_fields.go) copies
// it.Path directly into fi.Path, synthesizing sectionTitle + "." + it.Label
// only when it.Path is empty. A synthesized path (".VpcId", "General.VpcId")
// is not the real fieldpath.ExtractFieldList path any later reader expects.
//
