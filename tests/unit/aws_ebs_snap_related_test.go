...
func TestRelatedDemo_EBSSnap_Registered(t *testing.T) {
	_ = demo.GetResources
	checker := resource.GetRelatedDemo("ebs-snap")
	if checker == nil {
		t.Fatal("no demo checker registered for ebs-snap")
	}

	results := checker(resource.Resource{ID: "snap-0a1b2c3d4e5f60001"})
	if len(results) == 0 {
