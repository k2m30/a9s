...
// resource.IsRelatedActionable(count, approximate, hasFetchFilter, loading,
// hasErr) — the SAME predicate buildDetailRelatedBlocks uses to set
// RelatedBlock.Actionable (internal/app/detail_body.go:435). A row is dimmed
// (non-actionable) when: loading, or has an error, or Count==0, or
// (Count==-1 && no FetchFilter). It is actionable when it has a FetchFilter,
// or is Approximate, or Count>0.
//
