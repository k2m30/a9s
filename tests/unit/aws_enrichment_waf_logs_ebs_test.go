...
// (EnrichmentParallelism goroutines), so concurrent writers are possible.
...
func TestEnrichWAFLogging_ListResourcesErrorMarksRowTruncatedIDNotBadge(t *testing.T) {
	...
	// one of its calls — here the association read is what was refused, and a
	// label naming the logging read would point the operator at the call that
	// answered. The type is not in the label either: it comes from the
	// registry key at the surface, and a type here would render twice
	// ("enrich waf: waf: ...").
