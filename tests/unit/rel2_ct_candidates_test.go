...
// An event that names a secret by its full ARN is
// confirmed against the secrets list's own ARN, and the fragment rule that
// confirms a path-named role holds on two demo roles.
...
// TestRel2SecretNamedByNameStillResolves: an event naming a secret by its
// plain name resolves by id alongside the ARN rule.
...
// rel2SecondPathNamedRole is a second demo role under an IAM path, so the
// fragment rule is exercised by more than one fixture. Its event is
// evt-iam-assume-002.
const rel2SecondPathNamedRole = "acme-partner-integration-role"

// TestRel2PathNamedRoleResolvesFromItsEvent: an event names a role by an ARN
// whose resource part carries a path, and the id the panel navigates by is the
// bare role name the roles list holds.
...
// second path-named role. One fixture can pass by a coincidence of its own
// spelling; two keep the fragment rule honest.
