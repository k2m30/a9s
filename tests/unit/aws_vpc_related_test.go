...
// TestRelatedDemo_VPC_Registered verifies the demo checker is registered and
// returns valid results with all expected target types present.
func TestRelatedDemo_VPC_Registered(t *testing.T) {
	_ = demo.GetResources // ensure demo package is initialized
	checker := resource.GetRelatedDemo("vpc")
	if checker == nil {
		t.Fatal("no demo checker registered for vpc")
	}

	// Use the known fixture ID that returns a non-zero count.
	src := resource.Resource{ID: "vpc-0abc123def456789a"}
	results := checker(src)
	if len(results) == 0 {
