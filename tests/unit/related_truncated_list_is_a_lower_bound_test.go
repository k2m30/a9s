...
var truncatedUnknownAllowed = map[string]string{
	...
	"ecs_svc_related.go:183":        "tg is the join list; the elb target is unread until the load balancer ARNs resolve",
	"ecs_task_related_extra.go:270": "eni is the join list; the sg target is unread until the group ids resolve",
