...
var truncatedUnknownAllowed = map[string]string{
	...
	"ecs_task_related_extra.go:240": "eni is the join list; the sg target is unread until the group ids resolve",
	"eip_related.go:151":            "ec2 is the join list; asg is never fetched, the name comes off an instance tag",
