...
// which both register one (see core/aws/catalog_security.go).
//
...
// core/aws/catalog_security.go:141.
...
// already has FetchByIDs registered (core/aws/catalog_security.go:141).
...
// does — verified directly against core/aws/catalog_security.go, which
...
// authoring, core/aws/iam_roles.go has no FetchRoleByIDs-shaped function
// to invoke (only FetchIAMRolesPage, which paginates ALL roles, exists), and
// core/aws/iam_interfaces.go's IAMGetRoleAPI (GetRole) is declared but not
...
// fix is expected to add. Once core/aws/catalog_security.go registers
