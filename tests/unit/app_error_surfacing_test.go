...
// messages.FlashMsg with IsError=true is emitted containing the expected
...
// BatchMsg for a messages.FlashMsg. Returns (flash, true) on first hit.
func walkForFlash(cmd tea.Cmd) (messages.FlashMsg, bool) {
	if cmd == nil {
...
func findFlashInWalk(msg tea.Msg) (messages.FlashMsg, bool) {
	switch v := msg.(type) {
...
func TestAvailabilityPrefetchedMsg_PrefetchErrEmitsFlash(t *testing.T) {
	m := newTestModel()
	errMsg := messages.AvailabilityPrefetchedMsg{
...
func TestAvailabilityPrefetchedMsg_NilPrefetchErrNoFlash(t *testing.T) {
	m := newTestModel()
	cleanMsg := messages.AvailabilityPrefetchedMsg{
		Entries:        map[string]int{},
		Truncated:      map[string]bool{},
		IssueCounts:    map[string]int{},
		IssueTruncated: map[string]bool{},
...
func TestEnrichmentCheckedMsg_NilErrNoFlash(t *testing.T) {
	m := newTestModel()
	okMsg := messages.EnrichmentCheckedMsg{
		ResourceType: "sfn",
