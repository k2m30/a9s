...
func TestProfilePassedToInit(t *testing.T) {
	state := app.NewAppState("staging-admin", "")

	if state.ActiveProfile != "staging-admin" {
		t.Errorf("ActiveProfile should be 'staging-admin', got %q", state.ActiveProfile)
	}
	...
	if initMsg.Profile != "staging-admin" {
		t.Errorf("InitConnectMsg.Profile should be 'staging-admin', got %q", initMsg.Profile)
	}
}
...
func TestS3FetchUsesCorrectProfile(t *testing.T) {
	...
	state := app.NewAppState("staging-admin", "eu-central-1")
	state.Width = 80
	state.Height = 24
	...
	if state.Clients == nil {
		t.Skip("No AWS credentials available for profile staging-admin")
	}
