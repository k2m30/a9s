...
// TestRelated_Backup_Stubs verifies all 1 related def is registered as a stub (nil checker).
func TestRelated_Backup_Stubs(t *testing.T) {
	...
	for target, wantDisplay := range expected {
		...
		for _, def := range defs {
			if def.TargetType == target {
				found = true
				if def.Checker != nil {
					t.Errorf("backup %q: Checker should be nil (stub)", target)
				}
