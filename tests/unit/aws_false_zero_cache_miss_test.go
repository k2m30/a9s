...
// resource.UnknownRelated, renders "?"), never a definitive 0: a missing
// cache is not an authoritative answer. A PRESENT-but-empty cache entry
// (entry exists, zero resources) still returns a definitive 0.
package unit_test
...
// resources, IsTruncated=false) still returns a definitive Count:0.
...
// client), not the cache miss this test isolates.
