...
**Spec version check.** Confirm the spec has §7 "Authorized list columns" and §8 "Visual acceptance rules". If either is missing, the spec was generated by an older template — stop and re-run `a9s-resource-spec <shortName>` to regenerate.
...
There is no global "are the 5 surfaces wired?" preflight. The renderer already wires all five (S1 in `mainmenu.go`, S2 via `typeDef.ResolveColor`, S3 in `table_render.go`, S4 via the fetcher's `Status` field, S5 via `EnrichmentFinding.Summary`). The per-resource scenario harness test in phase 8 asserts end-to-end behavior and catches any silent failure — for THIS resource, against THIS fixture set — without relying on a global health check that was never the right abstraction.
...
Also read **`.a9s/views/<shortName>.yaml`** and **`internal/config/defaults.go`** (the `defaultViews` section for this shortName) and diff against spec §7 "Authorized list columns":

- Every column in the yaml that is not in spec §7 is **invented UI** and goes on the coder's delete list in phase 7.
- Every column in spec §7 that is missing from the yaml goes on the coder's add list in phase 7.
- Every backing `FieldPath` or `key` in the yaml must match the `Backing field key` column in spec §7. Mismatches are impl-plan deltas.

Record the column delta in the impl-plan's "Contract surface gap analysis" section alongside the related/enricher deltas.
...
- internal/aws/<shortName>_issue_enrichment.go — Wave 2 enricher per §3.2 (all enricher body lives here — do NOT spin off `<shortName>_maintenance.go` / `<shortName>_overdue.go` / similar helper files)
- internal/aws/<shortName>_detail_enrichment.go — only if §2 of the spec requires detail fields beyond the list shape
- .a9s/views/<shortName>.yaml — regenerate via `go run ./cmd/viewsgen/` AFTER amending `internal/config/defaults.go` to match spec §7
- internal/config/defaults.go — update the `defaultViews` entry for this shortName per spec §7 authorized columns
...
- Phase 6b approved: `tests/unit/aws_<shortName>_test.go`, `tests/unit/aws_<shortName>_issue_enrichment_test.go`, `tests/unit/aws_<shortName>_detail_enrichment_test.go` (if applicable), `tests/unit/aws_<shortName>_related_test.go`.
- Phase 7 approved: `internal/aws/<shortName>.go`, `internal/aws/<shortName>_interfaces.go`, `internal/aws/<shortName>_related.go`, `internal/aws/<shortName>_issue_enrichment.go`, `internal/aws/<shortName>_detail_enrichment.go` (if the spec demanded one), `.a9s/views/<shortName>.yaml`, `internal/config/defaults.go`.
...
Unit tests assert on `Resource.Status` and `EnrichmentFinding`. They do NOT verify that the rendered list view matches spec §4 — a fetcher can return the right `Status` while the view misreads it, or a `.a9s/views/<shortName>.yaml` can declare an unauthorized column. Phase 8 closes that gap by asserting on the actual rendered output via the scripted scenario harness (`tests/integration/SCENARIO_HARNESS.md`).
...
The test drives the real `tui.Model.Update()` loop via `fullIntegrationNewDemoScenario(t)`, navigates to the resource list and each fixture's detail view, and asserts the 6 rules from spec §8 "Visual acceptance":

1. **Column set exactness**: `scenario.ExpectFrameContains("<shortName>(")` after `OpenList(<shortName>)`; then for each column title in spec §7, `scenario.ExpectViewContains(<title>)`. For each column-title string that is NOT in §7 (e.g. `"CIS"`, `"Flags"`, `"Policy"`) assert `ExpectViewNotContains(<title>)`.
2. **Healthy rows blank S4**: `scenario.OpenList(<shortName>)`, then for each fixture whose spec §3 state bucket = Healthy, `scenario.ExpectRowStatusBlank(<fixture ID>)`. Banned render strings: `OK`, `ACTIVE`, `available`, `running`, `healthy`, `-`. Any presence fails.
...
5. **S1 menu count**: `scenario.ExpectMenuIssueCount(<expected N>)` where N = number of fixtures with `!` finding.
...
- columns: <N> in spec §7, <N> in rendered view (match: yes)
