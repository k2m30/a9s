...
- **A stale pin never blocks a round.** When a spec row deletes behaviour an existing test asserts, dev inverts that test in the same round, with a comment stating the correct behaviour as a fact (no task, row, ruling or "inverted" wording — that belongs in the commit and the log), and lists it in the log; acceptance reads the inversion.
