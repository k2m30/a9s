...
- **A spec is at most eight rows at dispatch**, each a backlog row with a witness, and the header names the count. More than eight, or an estimate above size `L` in `docs/development-process.md`, splits into two tasks before QA starts. The stop hook refuses a dev `DONE` at round 4 or later until a facilitator ruling is in the log.
...
6. Delete the batch branch and worktree.
