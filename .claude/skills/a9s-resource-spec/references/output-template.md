...
Every signal from §3.1 and §3.2 lands on the surfaces S1–S5 that `docs/attention-signals.md § Visualization Surfaces` defines; that section is where the wave→surface mapping lives. Do not restate the table or the mapping here; the §4 table names the surfaces per signal.
