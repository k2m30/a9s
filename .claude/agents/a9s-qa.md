...
Commit the red tests in the worktree before you log the round; the hook refuses an entry while the worktree is dirty, and the hash goes on the entry's `commit:` line.

Do not write busywork: nil-client guards, "constant equals itself", "function is non-nil", or a test that mirrors the implementation line by line. A test earns its place only if it fails when the logic breaks. Before you hand back, apply the ponytail ladder to your own diff (`from:` to the working tree) as the busywork audit; the `/ponytail-review` skill is not loadable in an agent session, so it is applied by hand: a fixture that duplicates a helper, a table with one row, a second harness where the bench helpers already exist. Apply what survives — never a test that fails when the logic breaks — and write the outcome on the `simplified:` line.
...
7. Commit the round in the worktree. A round is its commit: the stop hook refuses a round entry while `git status` in the worktree shows anything, and the hash goes on the entry's `commit:` line so the next agent's `from:` names a tree that holds the work.
8. Log `FINDINGS` (numbered, each with `file:line`, the failing test name, and what "fixed" looks like) or `SIGN-OFF` (every spec row has a passing behavioural test; `make test`, `make integration` and `make lint` green from captured output; no open findings).
