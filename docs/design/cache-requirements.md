...
Neither half of a result may hold the controller lock for its rows. The write half freezes its inputs and performs the disk write after releasing the lock (`queueCacheWrite`/`flushCacheWrites`); the read half freezes its inputs, builds the list body outside the lock (`listBodyBuild`), and takes the lock back only to swap the built body in. A key press waits for a pointer assignment, not for a row pass, so the wait does not grow with the size of the result. What makes the off-lock build sound is that a row's contents are REPLACED and never written into — `Fields`, `Findings` and `AttentionDetails` are each swapped for a fresh allocation by every writer that touches them — so a shallow copy of a row set is a stable view of it.
