...
**C7a — At-rest security seam.** Today the file is plaintext with owner-only permissions (0600 file, 0700 directory) — it contains real resource names. Encryption or obfuscation is a planned NEXT step, and the architecture accommodates it now: every read and write of the cache file goes through ONE encode/decode pair inside the cache module — no other code touches the file's bytes or path — and the file starts with a format marker (the schema version), so an encrypted format is just a new marker handled at that single point. Old plaintext files then simply read as "no cache" (C7's normal degradation) — no migration machinery. Key sourcing/management is explicitly out of scope until that step.
