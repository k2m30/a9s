...
Neither half of a result may hold the controller lock for its rows, and neither may sit in its caller's latency. The write half freezes its inputs under the lock and hands them to the save writer's own goroutine (`queueCacheWrite` stages, `runCacheWriteLoop` performs), so the fetch that triggered a save never waits for a whole row set to be marshalled; the read half freezes its inputs, builds the list body outside the lock (`listBodyBuild`), and takes the lock back only to swap the built body in. Reading a screen whose body memo already answers takes the READ lock, so two readers of one session overlap instead of queueing. A key press waits for a pointer assignment, not for a row pass, so the wait does not grow with the size of the result. What makes the off-lock build sound is that a row's contents are REPLACED and never written into — `Fields`, `Findings` and `AttentionDetails` are each swapped for a fresh allocation by every writer that touches them — so a shallow copy of a row set is a stable view of it.
