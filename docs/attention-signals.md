...
| `tg` | Target Groups | wave2 | `tg.all-targets-unhealthy` | all <N> targets unhealthy | broken | Every registered target is failing its health check, so the load balancer has nowhere to send a request and whatever sits in front of this group is down. Check the targets themselves, then the health-check path, port and matcher the group is configured with. |
...
| `dbi` | DB Instances | wave1 | `dbi.ca-cert-expiring-urgent` | server certificate expires in <N> days | broken | The server certificate expires within a month, and every client that verifies the connection will refuse to talk to the instance the moment it does. Book the maintenance window now and rotate the instance onto the current certificate authority. |
