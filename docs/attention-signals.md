...
| `iam-user` | IAM Users | `PasswordLastUsed` absent AND `CreateDate` >90d → Warning (dormant console user) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06) | Per user: `GetLoginProfile`, `ListMFADevices`, `ListAccessKeys`. Console password present AND no MFA device → Broken. `Active` key with `CreateDate` >90d → Warning (rotation). `GetAccessKeyLastUsed` dormant-key checks (`LastUsedDate` >90d / never used) → Warning — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | Credential report (`GenerateCredentialReport` + `GetCredentialReport`) — async/polling, provides all of the above in one account-wide pull when cached | [ListUsers](https://docs.aws.amazon.com/IAM/latest/APIReference/API_ListUsers.html) |
| `iam-group` | IAM Groups | None — `ListGroups` is config-only | `GetGroup` + `ListAttachedGroupPolicies` + `ListGroupPolicies` per group: no members → Warning (orphan); no attached AND no inline policies → Warning (no-op group); both fold into one `iam-group.orphan-or-noop` finding (no age gate) | `ListAttachedGroupPolicies` per group (admin-access + blast radius) | [GetGroup](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetGroup.html) |
| `waf` | WAF Web ACLs | None — `ListWebACLs` is config-only | `GetLoggingConfiguration` per ACL: no logging configuration → Warning; `ListResourcesForWebACL` empty → Warning (orphan — not associated with any resources); both fold into one `waf.no-logging` finding. `GetWebACL` feeds the `rules_summary` column only: `Rules==[]` → Warning (no-op ACL) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19); `DefaultAction==Allow` with zero rules → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | CloudWatch `BlockedRequests` spike; managed-rule-group version drift | [GetWebACL](https://docs.aws.amazon.com/waf/latest/APIReference/API_GetWebACL.html) |
