...
|---|---|---|---|---|---|
| `ec2` | EC2 Instances | `State.Name`: `running`→Healthy; `pending`/`shutting-down`/`stopping`→Warning; `stopped`→Warning; `terminated`→Dim. `StateReason.Code` begins `Server.*` on a `stopped` instance → Broken. `StateTransitionReason` carrying a user-initiated date >30d ago → Warning (long-stopped). instance metadata answers without a session token → Warning (IMDSv1 allowed — SSRF class). `PublicIpAddress` set → Warning (public address); neither fires on a terminated or shutting-down instance | `DescribeInstanceStatus(IncludeAllInstances=true)`: `SystemStatus.Status`/`InstanceStatus.Status` == `impaired` → Broken; == `initializing` → Warning (checks have not yet passed since start); == `insufficient-data` → Warning (AWS cannot determine); == `not-applicable` → Healthy (informational only, not surfaced); `Events[]` with scheduled retirement/reboot in ≤7d → Warning. Cache-scan cross-ref `sg` (zero API calls): a running instance with a public address whose security groups carry `wide_open` or a dangerous-port verdict → Broken (ports reachable from the internet); an unloaded `sg` cache is silence, never a clean bill. `DescribeInstanceAttribute(userData)` per instance through `secretscan` → Broken (credential in user data) | CloudWatch `StatusCheckFailed` | [DescribeInstances](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeInstances.html) |
| `ecs-svc` | ECS Services | `status`: `ACTIVE`→Healthy; `DRAINING`→Warning; `INACTIVE`→Broken. `desiredCount > 0` AND `runningCount == 0` → Broken (`no tasks running`); `runningCount < desiredCount` → Warning (`running below desired count`) | `DescribeServices`: `deployments[].rolloutState==FAILED` → Broken; `runningCount < desiredCount` AND no active `IN_PROGRESS` deployment → Broken; `events[]` containing `unable to place` / `ELB health checks failed` in last 10m → Broken; deployment circuit-breaker triggered → Broken; `networkConfiguration.awsvpcConfiguration.assignPublicIp == ENABLED` → Warning (tasks get public addresses) | CloudWatch `CPUUtilization`/`MemoryUtilization` p99 per service | [DescribeServices](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeServices.html) |
| `ecs` | ECS Clusters | `status`: `ACTIVE`→Healthy; `PROVISIONING`/`DEPROVISIONING`→Warning; `FAILED`/`INACTIVE`→Broken | `DescribeClusters(include=STATISTICS)`: `pendingTasksCount>0` → Warning; `runningTasksCount==0 && registeredContainerInstancesCount>0` → Warning (both fold into one `ecs.cluster-issue` finding) | CloudWatch `CPUReservation`/`MemoryReservation`; `DescribeContainerInstances` per cluster for agent-disconnect | [DescribeClusters](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeClusters.html) |
| `ecs-task` | ECS Tasks | `lastStatus`: `RUNNING`→Healthy; transitional states → Warning; `STOPPED` with `StopCode != UserInitiated`→Broken. `healthStatus==UNHEALTHY`→Broken | `DescribeTasks`: `StopCode` in `TaskFailedToStart`/`EssentialContainerExited` → Broken; container `exitCode` non-zero with `essential=true` → Broken. `DescribeTaskDefinition` once per distinct definition: any container `privileged` → Broken; `networkMode==host` or `pidMode==host` → Warning; any container without `readonlyRootFilesystem` → Warning; any container without `logConfiguration` → Warning; container `environment[]` through `secretscan` → Broken (credential in container environment) | Cross-cluster outlier detection | [DescribeTasks](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeTasks.html) |
| `lambda` | Lambda Functions | `State`: `Active`→Healthy; `Pending`→Warning; `Inactive`→Dim; `Failed`→Broken. `LastUpdateStatus==Failed`→Broken. `Runtime` in [deprecated-runtimes list](https://docs.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html) → Broken. `DeadLetterConfig==nil` → Warning. `Environment.Variables` through `secretscan` → Broken (credential in environment variables) | `GetPolicy` per function through `iampolicy` — a wildcard principal with no restrictive condition → Broken (invokable by anyone); `ResourceNotFoundException` means no policy, which is healthy. `ListFunctionUrlConfigs` per function — any config with `AuthType == NONE` → Broken (function endpoint open without authentication) | CloudWatch `Errors/Invocations` ratio; `Throttles`; `Duration` p99 vs `Timeout`; `GetFunctionConcurrency` per function | [GetFunctionConfiguration](https://docs.aws.amazon.com/lambda/latest/api/API_GetFunctionConfiguration.html) |
| `asg` | Auto Scaling Groups | `Status==""`→Healthy; `Delete in progress`→Warning. `Instances[].HealthStatus==Unhealthy` → Warning. InService count < `MinSize` → Broken. `SuspendedProcesses` containing `Launch`/`Terminate`/`HealthCheck` → Warning. `LaunchConfigurationName` set → Warning (legacy launch configuration). Fewer than two `AvailabilityZones` → Warning. Attached to a load balancer or target group with `HealthCheckType != ELB` → Warning | `DescribeScalingActivities(MaxRecords=1)`: latest `StatusCode==Failed` → Broken (launch-failure loop). One batched `DescribeLaunchConfigurations` for the groups that still reference one: `MetadataOptions` absent or `HttpTokens != required` → Warning (IMDSv1 allowed; a launch configuration without the block defaults to optional); `AssociatePublicIpAddress == true` → Warning; `UserData` through `secretscan` → Broken (credential in user data) | CloudWatch `GroupDesiredCapacity` vs `GroupInServiceInstances` delta sustained | [DescribeAutoScalingGroups](https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_DescribeAutoScalingGroups.html) |
| `eb` | Elastic Beanstalk | `Health`: `Green`→Healthy; `Yellow`/`Grey`→Warning; `Red`→Broken. `Status==Terminated`→Dim | `DescribeEnvironmentHealth`: `Causes[]` non-empty → Warning detail. `DescribeConfigurationSettings` per environment: managed platform updates not enabled → Warning (`managed platform updates off`); health reporting not `enhanced` → Warning (`enhanced health reporting off`); log streaming not enabled → Warning (`log streaming to CloudWatch off`) | `DescribeConfigurationSettings` platform-EOL check; `DescribeEvents` severity filter | [DescribeEnvironments](https://docs.aws.amazon.com/elasticbeanstalk/latest/api/API_DescribeEnvironments.html) |
| `ebs` | EBS Volumes | `State`: `in-use`→Healthy; `creating`/`deleting`→Warning; `error`→Broken. `available` with `CreateTime` >7d → Warning (orphan). `Encrypted==false` → Warning (unencrypted at rest) | `DescribeVolumeStatus` (account-wide, paginated): `VolumeStatus.Status != ok` → Broken (single `ebs.volume-io-degraded` finding; the most recent event and action code surface as detail rows). Plus a cache-only join against the loaded backup plan list: no selection matching the resource's ARN or its tags → Warning (`not covered by a backup plan`). The join answers only from a list read to the end — an unfetched or cut-short list reports nothing, since a plan on an unread page may cover it. A second cache-only join against the loaded snapshot list: an in-use volume with no snapshot of it → Warning (`no snapshot exists`) | CloudWatch `VolumeQueueLength`; `BurstBalance` on gp2 | [DescribeVolumeStatus](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeVolumeStatus.html) |
| `ebs-snap` | EBS Snapshots | `State`: `completed`→Healthy; `pending`→Warning; `error`→Broken; `recoverable`/`recovering`→Broken. Age >365d with automated description → Warning (cost). `Encrypted==false` → Warning (unencrypted at rest) | Cache-scan (zero API calls): cross-ref `ebs` — source volume deleted → Warning (orphan). One account-wide `DescribeSnapshots(OwnerIds=[self], RestorableByUserIds=[all])` names every snapshot restorable by every account → Broken (shared with all AWS accounts); its failure marks every row uninspected | Snapshot-lineage cost attribution | [DescribeSnapshots](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeSnapshots.html) |
| `ami` | AMIs | `State`: `available`→Healthy; `pending`/`transient`→Warning; `failed`/`error`/`invalid`→Broken; `deregistered`/`disabled`→Dim. `DeprecationTime < now()` → Warning. Cross-ref `ebs-snap` (owner-scoped only — skip public/marketplace AMIs) — backing snapshot missing → Warning — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06). `Public == true` → Broken (shared with all AWS accounts; the fetcher asks for `Owners=self`, so this is an image the account owns) | None | Sharing-history audit | [DescribeImages](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html) |
| `lt` | Launch Templates | None — `DescribeLaunchTemplates` carries no health data. `DefaultVersionNumber != LatestVersionNumber` is DISPLAY ONLY (live-witnessed as the healthy steady state; flagging it is alarm fatigue). "Who references this template" is answered by the related panel (`asg`∪`ng`∪`ec2` cache cross-ref pivots, zero extra calls; unknown — never 0-as-fact — when a sibling cache is truncated), not a list column. No unused-template flag (templates are free and inert) | `DescribeLaunchTemplateVersions(Versions=["$Default"])` per template — the one extra call, reading what `asg`/`ng`/`ec2` actually resolve (`$Latest` is staging): `MetadataOptions.HttpTokens != "required"` (unset defaults to `optional`!) → Warning (IMDSv1 allowed — SSRF class). Explicit `BlockDeviceMappings[].Ebs.Encrypted == false` → Warning (nil is UNKNOWN, not unencrypted — default-encryption accounts make nil legitimate; never flag nil). `ImageId` present in the loaded `ami` cache AND its `DeprecationTime` past → Warning (deprecated AMI; not-in-cache ≠ deregistered — public/marketplace AMIs legitimately absent; skip `resolve:ssm:` refs). The `$Default` version's `UserData`, base64-decoded through `secretscan` → Broken (credential in user data) | Version diffing (`$Default` vs `$Latest`); `IamInstanceProfile`→role resolution; `ssm:GetParameter` AMI resolution; explicit `DescribeImages` checks; `DescribeInstances`-by-tag; spot/CPU/placement deep config | [DescribeLaunchTemplateVersions](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeLaunchTemplateVersions.html) |

### Containers

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `eks` | EKS Clusters | Posture, from the `DescribeCluster` the fetcher already makes: `resourcesVpcConfig.endpointPublicAccess` → Broken when the public CIDR list is empty or contains `0.0.0.0/0`, Warning when scoped; enabled `logging.clusterLogging[]` types not covering all five → Warning; no `encryptionConfig` entry covering `secrets` → Warning; `version` reported by `DescribeClusterVersions` (one call per page) as anything but standard support → Broken | `DescribeCluster` per cluster (N+1): `status`: `ACTIVE`→Healthy; `CREATING`/`UPDATING`/`DELETING`/`PENDING`→Warning; `FAILED`→Broken. `health.issues[]` non-empty on an otherwise-healthy cluster → Warning (issue codes fold into the phrase; ranked below the lifecycle-state findings) | addon health | [DescribeCluster](https://docs.aws.amazon.com/eks/latest/APIReference/API_DescribeCluster.html) |
| `ng` | EKS Node Groups | None — `ListNodegroups` returns node-group name strings only | `DescribeNodegroup` per node group (N+1): `status`: `ACTIVE`→Healthy; `CREATING`/`UPDATING`/`DELETING`→Warning; `CREATE_FAILED`/`DELETE_FAILED`/`DEGRADED`→Broken. `health.issues[]` codes (`InsufficientFreeAddresses`, `Ec2LaunchTemplateVersionMismatch`, `AutoScalingGroupInvalidConfiguration`, `AccessDenied`, …) fold into the Broken state findings; issues on an `ACTIVE` node group surface as fields only (no finding) | AMI release drift; `ListUpdates` per node group | [DescribeNodegroup](https://docs.aws.amazon.com/eks/latest/APIReference/API_DescribeNodegroup.html) |

### Networking

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `elb` | Load Balancers | ELBv2 only: `State.Code`: `active`→Healthy; `provisioning`/`active_impaired`→Warning; `failed`→Broken. Surface `State.Reason` as Broken detail. Classic (ELBv1) has no `State` field | `DescribeLoadBalancerAttributes` per LB: `deletion_protection.enabled==false` or `access_logs.s3.enabled==false` → Warning (one `elb.misconfigured` finding; both-off is the AWS create default and never escalates to Broken). ALB `routing.http.desync_mitigation_mode==monitor` → Warning; ALB `routing.http.drop_invalid_header_fields.enabled!=true` → Warning (an attribute the API did not report is unknown, not off). `DescribeListeners` per LB: ALB `HTTP` listener whose default actions do not all redirect to HTTPS, or NLB `TCP` listener on 443 → Warning; `HTTPS`/`TLS` listener on a policy outside the `TLS13-`/`TLS-1-2-`/`FS-1-2-` families → Warning. Target health lives on `tg` | CloudWatch `HTTPCode_ELB_5XX_Count` | [DescribeLoadBalancers](https://docs.aws.amazon.com/elasticloadbalancing/latest/APIReference/API_DescribeLoadBalancers.html) |
| `tg` | Target Groups | `LoadBalancerArns==[]` → Warning (orphan) | `DescribeTargetHealth` per TG: any target `State==unhealthy` → Warning; all reporting targets unhealthy → Broken (`initial`/`draining`/`unused` do not count toward the numerator) | CloudWatch `UnHealthyHostCount` / `HealthyHostCount` ratios | [DescribeTargetHealth](https://docs.aws.amazon.com/elasticloadbalancing/latest/APIReference/API_DescribeTargetHealth.html) |
...
| `vpce` | VPC Endpoints | `State`: `Available`→Healthy; `PendingAcceptance`/`Pending`/`Deleting`→Warning; `Failed`/`Rejected`/`Expired`→Broken; `Partial`→Broken (Interface endpoint with some AZ ENIs failed to provision); `Deleted`→Dim. `LastError` non-empty → Broken detail. Interface endpoint `NetworkInterfaceIds==[]` → Broken. Gateway endpoint `RouteTableIds==[]` → Warning (orphan). `PolicyDocument` through `iampolicy` — a wildcard principal holding a wildcard action with no restrictive condition → Warning (the full-access policy AWS attaches when none is supplied); a deleting/deleted endpoint emits no posture finding | None | Endpoint policy analysis beyond the wildcard-principal case | [DescribeVpcEndpoints](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeVpcEndpoints.html) |
| `tgw` | Transit Gateways | `State`: `available`→Healthy; `pending`/`modifying`/`deleting`→Warning; `deleted`→Dim. (Real attachment-level failures surface via Wave 2.) `Options.AutoAcceptSharedAttachments==enable` → Warning (any shared account can attach without review); a deleting/deleted gateway emits no posture finding | `DescribeTransitGatewayAttachments`: any attachment `State` in `failed`/`failing` → Broken; `modifying`/`pendingAcceptance`/`rollingBack` → Warning (worst severity wins — one finding per TGW). `rejected`/`rejecting` → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | CloudWatch `PacketDropCountBlackhole`/`PacketDropCountNoRoute` | [DescribeTransitGatewayAttachments](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeTransitGatewayAttachments.html) |
| `transfer` | Transfer Family | `State` (ListServers): `ONLINE`→Healthy; `OFFLINE`→Warning (partner-facing endpoint not accepting transfers); `STARTING`/`STOPPING`→Warning (transient); `START_FAILED`→Broken; `STOP_FAILED`→Warning. `LoggingRole` nil is NOT a Wave-1 signal (structured logging makes nil legitimate — defer to Wave 2). `UserCount==0` is noise (AS2/external-IdP servers legitimately 0) — column only. `Protocols` absent from ListedServer — AS2-vs-SFTP unknowable at Wave 1 | `DescribeServer` per server (N+1; accounts run 1–5): `SecurityPolicyName` in a denylist of known-weak legacy policies (`TransferSecurityPolicy-2018-11`, `-2020-06`) → Warning (weak ciphers / old TLS; no false positives on FIPS/restricted variants). `LoggingRole == nil` AND `len(StructuredLogDestinations)==0` → Warning (no activity logging — audit gap for a B2B endpoint). Agreements child rows: `Status == INACTIVE` → Warning (partner traffic rejected). Certificate child rows: `InactiveDate` past or `Status == INACTIVE` → Broken, within 30d → Warning (expiry surfaces on cert rows, never bubbled account-wide to the server) | CloudWatch `FilesIn`/`FilesOut`/`BytesIn`/`BytesOut`, AS2 `InboundMessage`/`OutboundMessage`; `ListExecutions` workflow runs; per-user home dirs/SSH keys; AS2 MDN history | [DescribeServer](https://docs.aws.amazon.com/transfer/latest/userguide/API_DescribeServer.html) |
| `vpc-peer` | VPC Peering | `Status.Code`: `active`→Healthy; `provisioning`/`initiating-request`→Warning (transient); `pending-acceptance`→Warning — countdown to `ExpirationTime` is the actionable bit (AWS expires at 7d; escalate wording under 48h); `expired`→Warning; `rejected`/`failed`→Broken (surface `Status.Message` verbatim); `deleting`→Warning; `deleted`→Dim (AWS keeps it listed for a window). Cross-account / cross-region are FACTS (neutral badges), never signals. Active-only CIDR overlap (IPv4 `CidrBlockSet`×`CidrBlockSet` intersection) → Warning, ranked below missing-route | None (no per-connection describe exists). Derived zero-API cache-scan signals (the cache-scan enricher layer, snapshot_cross_ref.go precedent): active pcx with NO loaded-`rtb` route referencing it → Warning "no local route to peer" (guards: rtb cache present AND not truncated); route references the pcx but `Route.State == blackhole` → Warning "route to peer blackholed" | SG cross-referencing; IPv6 overlap; DNS effective-state/ClassicLink (SDK-deprecated); flow-logs traffic verification; remote-side health (invisible by construction); TGW-migration advice | [DescribeVpcPeeringConnections](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeVpcPeeringConnections.html) |
...
| `sg` | Security Groups | `IpPermissions[]` with `0.0.0.0/0` on any port in `sensitivePorts` (20, 21, 22, 23, 25, 445, 1433, 1521, 2483, 3306, 3389, 5432, 5601, 6379, 7199, 8888, 9092, 9160, 9200, 11211, 27017 — 8080/8443 deliberately excluded as too noisy) → Broken (exposed admin/db). `GroupName=="default"` carrying any ingress rule, or egress beyond the AWS-created allow-all → Warning | Cross-ref `eni` — a non-default group referenced by no ENI in the loaded, untruncated ENI list → Warning (orphan); an absent or first-page-only ENI list proves nothing and emits nothing. Zero AWS calls | SG-referencing-deleted-SG detection | [DescribeSecurityGroups](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeSecurityGroups.html) |

### Databases & Storage

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `dbi` | DB Instances | `DBInstanceStatus`: `available`→Healthy; transitional, including `deleting`, → Warning; `failed`/`storage-full`/`incompatible-*`/`restore-error`/`inaccessible-encryption-credentials`→Broken. `BackupRetentionPeriod==0` → Warning. `PubliclyAccessible==true` → Warning (reachable from the internet). `StorageEncrypted==false` → Warning (unencrypted at rest). `DeletionProtection==false` → Warning. `MultiAZ==false` on a non-Aurora primary → Warning (single-AZ). `AutoMinorVersionUpgrade==false` → Warning. `IAMDatabaseAuthenticationEnabled==false` on an engine that supports it → Warning. `MasterUsername` is a vendor default → Warning. `CertificateDetails.ValidTill` within 90 days → Warning, within 30 → Broken | `DescribePendingMaintenanceActions` (one account-wide call): any pending maintenance action → Warning (`maintenance scheduled`; no overdue gate — unlike `dbc`). `DescribeDBEngineVersions` once per distinct (engine, version) pair: version not listed as available → Broken (`engine version deprecated`). Plus a join against the loaded backup plan list: no selection matching the resource's ARN or its tags → Warning (`not covered by a backup plan`). The join answers only from a list read to the end — an unfetched or cut-short list reports nothing, since a plan on an unread page may cover it. When some plan selects by tag, `ListTagsForResource` reads the tags of each resource the selections' ARNs do not already cover, one call per resource and capped; a resource whose tags cannot be read reports nothing | CloudWatch `FreeStorageSpace`, `CPUUtilization`, `ReplicaLag`, `DatabaseConnections` | [DescribeDBInstances](https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_DescribeDBInstances.html) |
...
| `ddb` | DynamoDB Tables | None — `ListTables` returns table names only | `DescribeTable` per table (N+1): `TableStatus`: `ACTIVE`→Healthy; `CREATING`/`UPDATING`/`DELETING`/`ARCHIVING`→Warning; `INACCESSIBLE_ENCRYPTION_CREDENTIALS`/`ARCHIVED`→Broken. `DeletionProtectionEnabled` not true → Warning. Plus `DescribeContinuousBackups` per table: PITR disabled → Warning; and `GetResourcePolicy` per table: wildcard principal → Broken (`resource policy open to anyone`), otherwise a named foreign account → Warning. Plus a join against the loaded backup plan list: no selection matching the resource's ARN or its tags → Warning (`not covered by a backup plan`). The join answers only from a list read to the end — an unfetched or cut-short list reports nothing, since a plan on an unread page may cover it. When some plan selects by tag, `ListTagsOfResource` reads the tags of each resource the selections' ARNs do not already cover, one call per resource and capped; a resource whose tags cannot be read reports nothing | CloudWatch `ReadThrottleEvents`+`WriteThrottleEvents`, `SystemErrors` | [DescribeTable](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_DescribeTable.html) |
| `opensearch` | OpenSearch Domains | `DescribeDomains` (bounded fan-out, in the fetcher): `Deleted==true`→Dim and nothing else is reported for a domain on its way out; `DomainProcessingStatus==Isolated`→Broken; `Processing==true` or `UpgradeProcessing==true`→Warning; `ServiceSoftwareOptions.UpdateAvailable==true` with the automated update date already passed → Warning (`software update forced soon`); `EncryptionAtRestOptions.Enabled==false` → Warning; no `VPCOptions` while the access policy allows any principal → Broken (`reachable outside a VPC`); `DomainEndpointOptions.EnforceHTTPS` not true → Warning; `NodeToNodeEncryptionOptions.Enabled` not true → Warning | None — every signal is readable from the `DomainStatus` the fetcher already holds | Cluster-health (Red/Yellow/Green) is CloudWatch-only: `AWS/ES` namespace, `ClusterStatus.red`/`yellow`. Also `FreeStorageSpace`, `JVMMemoryPressure` | [DescribeDomains](https://docs.aws.amazon.com/opensearch-service/latest/APIReference/API_DescribeDomains.html) |
...
| `dbi-snap` | DB Instance Snapshots | `Status`: `available`→Healthy; `creating`→Warning; any other state the snapshot cannot yet be restored from, such as `copying`, → Warning; `failed`/`incompatible-*`→Broken. `Encrypted==false` → Warning (unencrypted at rest) | Cache-scan (zero API calls): cross-ref `dbi` — source DB deleted → Broken (orphan); when parent DB in the loaded list, snapshot age > parent `BackupRetentionPeriod` AND `SnapshotType==automated` → Broken (past retention). Plus `DescribeDBSnapshotAttributes` per snapshot: `restore` attribute lists the `all` group → Broken (`shared with all AWS accounts`) | None | [DescribeDBSnapshots](https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_DescribeDBSnapshots.html) |
| `dbc-snap` | DB Cluster Snapshots | `Status`: `available`→Healthy; `creating`→Warning; any other state the snapshot cannot yet be restored from, such as `copying`, → Warning; `failed`→Broken. Manual snapshot age >365d → Warning (cost) | Cache-scan (zero API calls): cross-ref `dbc` — source cluster deleted → Broken (orphan); when parent cluster in loaded list, age > cluster `BackupRetentionPeriod` AND automated → Broken (past retention). Plus `DescribeDBClusterSnapshotAttributes` per snapshot, on whichever of the rds / docdb clients owns the row: `restore` attribute lists the `all` group → Broken (`shared with all AWS accounts`) | None | [DescribeDBClusterSnapshots](https://docs.aws.amazon.com/documentdb/latest/developerguide/API_DescribeDBClusterSnapshots.html) |

### Messaging

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `sqs` | SQS Queues | None — `ListQueues` returns URLs only | `GetQueueAttributes(RedrivePolicy, VisibilityTimeout, KmsMasterKeyId, Policy)` per queue: `ApproximateNumberOfMessages` > threshold → Warning — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06), rising unbounded → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06); `ApproximateAgeOfOldestMessage` > `VisibilityTimeout`×5 → Warning (consumer lag) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06); is-DLQ with messages → Warning — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06); `RedrivePolicy` unset on main queue → Warning (`no DLQ configured`); `KmsMasterKeyId` unset → Warning (`not encrypted with KMS`, its own `sqs.no-kms` finding); access policy allowing a wildcard principal → Broken (`queue policy open to anyone`) | CloudWatch `NumberOfMessagesSent/Received` trend | [GetQueueAttributes](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_GetQueueAttributes.html) |
| `sns` | SNS Topics | None — `ListTopics` returns ARN only | `ListSubscriptionsByTopic` per topic: zero subscriptions → Warning (orphan topic); all subscriptions stuck `PendingConfirmation` → Warning. `GetTopicAttributes` per topic: access policy allowing a wildcard principal → Broken (`topic policy open to anyone`); `KmsMasterKeyId` absent or empty → Warning (`not encrypted with KMS`) | CloudWatch `NumberOfNotificationsFailed` | [GetTopicAttributes](https://docs.aws.amazon.com/sns/latest/api/API_GetTopicAttributes.html) |
| `sns-sub` | SNS Subscriptions | `SubscriptionArn == "PendingConfirmation"` → Warning (never confirmed); `SubscriptionArn == "Deleted"` → Dim (endpoint deleted); `Protocol == "http"` → Warning (`delivers over plain HTTP`) | None | `GetSubscriptionAttributes` per subscription (DLQ); CloudWatch `NumberOfNotificationsFailed` per endpoint | [ListSubscriptions](https://docs.aws.amazon.com/sns/latest/api/API_ListSubscriptions.html) |
...
| `msk` | MSK Clusters | `State`: `ACTIVE`→Healthy; `CREATING`/`UPDATING`/`MAINTENANCE`/`REBOOTING_BROKER`/`HEALING`→Warning; `DELETING`→Dim; `FAILED`→Broken | `DescribeClusterV2` per provisioned cluster (serverless skipped): Kafka version < 2.8 → Warning (`broker software outdated`); `EncryptionInfo.EncryptionInTransit.ClientBroker != TLS` → Warning (`encryption in transit not enforced`); brokers published with service-provided public addresses → Broken (`brokers reachable from the internet`); `ClientAuthentication.Unauthenticated.Enabled` → Broken (`unauthenticated access allowed`). Per-broker runtime state stays unobservable (`ListNodes` returns node metadata but no `RUNNING` enum) | CloudWatch `ActiveControllerCount`, `OfflinePartitionsCount`, `UnderReplicatedPartitions`, `KafkaDataLogsDiskUsed` | [ListClustersV2](https://docs.aws.amazon.com/msk/1.0/apireference/v2-clusters.html) |
| `sfn` | Step Functions | None — `ListStateMachines` is config-only | `ListExecutions(maxResults=1)` per state machine (EXPRESS machines skip this call alone — the API rejects them): latest execution `FAILED`/`TIMED_OUT`/`ABORTED` → Broken. `DescribeStateMachine` per state machine, EXPRESS included: logging level absent or `OFF` → Warning (`execution logging off`); encryption not a customer managed key → Warning (`not encrypted with a customer key`); a credential in the definition → Broken (`credential in state machine definition`) | CloudWatch `ExecutionsFailed`/`ExecutionsTimedOut`/`ExecutionThrottled` trend | [ListExecutions](https://docs.aws.amazon.com/step-functions/latest/apireference/API_ListExecutions.html) |
...
| `ses` | SES Identities | `VerificationStatus`: `SUCCESS`→Healthy; `PENDING`→Warning; `FAILED`/`TEMPORARY_FAILURE`/`NOT_STARTED`→Broken. `SendingEnabled==false` → Warning | `GetAccount` (SESv2, account-wide): `EnforcementStatus` in `PROBATION`/`SHUTDOWN` → Broken; `SendQuota.SentLast24Hours > 0.8 × Max24HourSend` → Warning. `GetEmailIdentity` per identity: a domain identity not signing its outbound mail → Warning (`DKIM not enabled`) | Reputation dashboard (`BounceRate`/`ComplaintRate`) via CloudWatch | [GetAccount (SESv2)](https://docs.aws.amazon.com/ses/latest/APIReference-V2/API_GetAccount.html) |

### Secrets & Config

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `secrets` | Secrets Manager | `RotationEnabled==true && now > NextRotationDate` → Warning (overdue). `RotationEnabled==true && (now - LastRotatedDate) > RotationRules.AutomaticallyAfterDays × 2` → Broken (rotation failing) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06). `LastAccessedDate` >180d → Warning (dormant; note: field is day-truncated and excludes access in the current call). `DeletedDate` set → Broken. When no state finding fires: `RotationEnabled==false` → Warning (rotation not enabled), else `LastChangedDate` >365d → Warning (value unchanged in over 365 days) | `GetResourcePolicy` per secret, evaluated through `core/iampolicy`: wildcard principal with no restrictive condition → Broken; otherwise a principal in another account → Warning. A secret with no resource policy attached is the healthy default | `DescribeSecret` per secret for `VersionIdsToStages` stuck on `AWSPENDING` | [ListSecrets](https://docs.aws.amazon.com/secretsmanager/latest/apireference/API_ListSecrets.html) |
...
| `ssm` | SSM Parameters | `Type==String` AND lowercased name ending in `_password`/`_secret`/`_token`/`_apikey`/`_api_key`/`_credentials`/`_passwd` → Broken (plaintext value looks like a credential; takes precedence). Any type with `LastModifiedDate` >365d → Warning (not modified in over 365 days). `Tier==Advanced` AND `LastModifiedDate` >90d → Warning (cost — aged Advanced parameter) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06) | None | `GetParameterHistory` per parameter (true access-age) | [DescribeParameters](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_DescribeParameters.html) |
| `kms` | KMS Keys | None — `ListKeys` returns `{KeyId, KeyArn}` only | `DescribeKey` per key: `KeyState`: `Enabled`→Healthy; `Creating`/`Updating`→Warning; `Disabled`→Warning; `PendingDeletion`/`PendingImport`/`PendingReplicaDeletion`→Broken; `Unavailable`→Broken. Plus `GetKeyRotationStatus` per key: `KeyRotationEnabled==false` on CMK → Warning. `GetKeyPolicy(default)` per key, evaluated through `core/iampolicy`: wildcard principal with no restrictive condition → Broken | Grant-level analysis per key (`ListGrants`) | [DescribeKey](https://docs.aws.amazon.com/kms/latest/APIReference/API_DescribeKey.html) |

### Security & IAM

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `role` | IAM Roles | `AssumeRolePolicyDocument` (URL-encoded JSON on `ListRoles`) evaluated through `core/iampolicy`: a wildcard principal with no restrictive condition (`sts:ExternalId` counts as restrictive here) → Broken. A confused-deputy service principal trusted with no `aws:SourceAccount`/`aws:SourceArn`/`aws:SourceOrgID` condition → Warning; service-linked roles and ec2-only instance roles are excluded. Inline policies (already walked for `policy_resources`) matching a known privilege-escalation action combination → Broken | `GetRole` per role: `RoleLastUsed.LastUsedDate` missing or >90d → Warning (dormant; field is region-scoped — may false-warn in multi-region accounts). `ListAttachedRolePolicies` per role: `AdministratorAccess` or `PowerUserAccess` attached → Warning | `GenerateServiceLastAccessedDetails` async permission-usage audit | [GetRole](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetRole.html) |
| `policy` | IAM Policies | `AttachmentCount==0` AND not AWS-managed → Warning (orphan) | `GetPolicyVersion` per customer-managed policy: document contains `"Effect":"Allow","Action":"*","Resource":"*"` → Broken (wildcard admin); otherwise a known privilege-escalation action combination → Broken (an admin policy is reported once, as admin) | IAM Access Advisor unused-permission analysis | [GetPolicyVersion](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetPolicyVersion.html) |
| `iam-user` | IAM Users | None — console-password presence is only knowable from `GetLoginProfile`, so every user signal is Wave 2 | Per user: `GetLoginProfile`, `ListMFADevices`, `ListAccessKeys`, `ListAttachedUserPolicies`, `GetAccessKeyLastUsed`. Console password present AND no MFA device → Broken. Console password present AND `PasswordLastUsed` absent AND `CreateDate` >90d → Warning (never used). Console password present AND `PasswordLastUsed` >90d ago → Warning (`console sign-in unused for 90 days`). `Active` key with `CreateDate` >90d → Warning (rotation). `Active` key whose `LastUsedDate` is >90d old, or which has never been used and is itself >90d old → Warning (key ID shown as its last four characters only). Two `Active` keys → Warning. `AdministratorAccess` or `PowerUserAccess` attached → Warning. Each condition is its own finding | Credential report (`GenerateCredentialReport` + `GetCredentialReport`) — async/polling, provides all of the above in one account-wide pull when cached | [ListUsers](https://docs.aws.amazon.com/IAM/latest/APIReference/API_ListUsers.html) |
| `iam-group` | IAM Groups | None — `ListGroups` is config-only | `GetGroup` + `ListAttachedGroupPolicies` + `ListGroupPolicies` per group: no members → Warning (orphan); no attached AND no inline policies → Warning (no-op group); both fold into one `iam-group.orphan-or-noop` finding (no age gate). `AdministratorAccess` or `PowerUserAccess` attached → Warning, as its own finding | Blast-radius analysis across the group's members | [GetGroup](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetGroup.html) |
| `waf` | WAF Web ACLs | None — `ListWebACLs` is config-only | `GetLoggingConfiguration` per ACL: no logging configuration → Warning; `ListResourcesForWebACL` empty → Warning (orphan — not associated with any resources); both fold into one `waf.no-logging` finding. `GetWebACL` per ACL: `Rules==[]` → Warning (no rules), as its own finding, and feeds the `rules_summary` column. `DefaultAction==Allow` with zero rules → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | CloudWatch `BlockedRequests` spike; managed-rule-group version drift | [GetWebACL](https://docs.aws.amazon.com/waf/latest/APIReference/API_GetWebACL.html) |

### DNS, CDN, Certs

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `r53` | Route 53 Hosted Zones | `ResourceRecordSetCount<=2` on non-new zone → Warning (only SOA+NS, likely unused) | `GetHostedZone` per zone: `Config.PrivateZone==true` with empty `VPCs[]` → Warning (orphan private zone). On public zones: `ListQueryLoggingConfigs` empty → Warning (query logging off); an `A` record whose public address is held by no elastic IP, instance or network interface → Broken (points at a released address), skipped whenever any of those three lists is absent or partial. `GetDNSSEC` per zone with `Config.PrivateZone==false` (private zones not supported): status `SIGNING` with KSK inactive → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06) | Registrar nameserver lookup (external); health-check status aggregation per zone | [ListHostedZones](https://docs.aws.amazon.com/Route53/latest/APIReference/API_ListHostedZones.html) |
| `cf` | CloudFront Distributions | None — colour derives from findings | `GetDistributionConfig` per distribution: `ViewerProtocolPolicy==allow-all` or an origin on `http-only` → Warning (insecure protocol); an S3 origin naming a bucket the account does not hold → Broken; `MinimumProtocolVersion` below TLS 1.2 → Warning; logging off, no default root object, an S3 origin with neither origin access control nor origin access identity, the default certificate on a distribution with aliases, and no geo restriction → Warning | CloudWatch `5xxErrorRate`/`TotalErrorRate`; origin-deleted cross-check | [ListDistributions](https://docs.aws.amazon.com/cloudfront/latest/APIReference/API_ListDistributions.html) |
| `acm` | ACM Certificates | `Status`: `ISSUED`→Healthy; `PENDING_VALIDATION`→Warning; `EXPIRED`/`REVOKED`/`FAILED`/`VALIDATION_TIMED_OUT`→Broken; `INACTIVE`→Dim. `NotAfter - now() < 30d` → Warning, `< 7d` → Broken. `InUse==false` on an `ISSUED` cert outside the expiry windows → Warning (orphan; expiry takes priority). `KeyAlgorithm` is RSA below 2048 bits → Warning (weak key algorithm) | `DescribeCertificate` per cert: `RenewalSummary.RenewalStatus==FAILED` → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06); `DomainValidationOptions[].ValidationStatus==FAILED` → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06) | None | [ListCertificates](https://docs.aws.amazon.com/acm/latest/APIReference/API_ListCertificates.html) |
| `apigw` | API Gateways | None — config-only list | `apigatewayv2:GetStages` per HTTP/WebSocket API: no deployed stage → Warning (orphan API); throttling burst/rate limit == 0 or `AccessLogSettings==nil` → Warning, folded into one `apigw.stage-config-issues` finding. `GetAuthorizers` on both lanes: no authorizer → Broken on a REST API whose endpoint is not private and whose resource policy is absent or public, Warning otherwise. REST v1 `GetStages`: no access logs, tracing off → Warning; a stage variable that scans as a credential → Broken. | CloudWatch `5XXError`/`4XXError`; `GetUsagePlans` quota-breach detection | [GetStages](https://docs.aws.amazon.com/apigateway/latest/api/API_GetStages.html) |

### Monitoring

| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `alarm` | CloudWatch Alarms | `StateValue`: `OK`→Healthy; `INSUFFICIENT_DATA`→Warning; `ALARM`→Broken. `AlarmActions==[]` (alert-to-nowhere) → Warning (`alarm.no_actions`). `ActionsEnabled==false` → Warning (`alarm.actions-disabled`), evaluated independently: an alarm can have actions wired and still have them switched off. `StateValue==INSUFFICIENT_DATA` AND `StateUpdatedTimestamp` older than 2×`Period` → Broken (metric pipeline dead) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06). Cross-ref siblings — `Dimensions` reference a resource ID absent from the already-loaded sibling-type list (skip rule when sibling list wasn't loaded in this sweep) → Warning (zombie alarm) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-06) | None | None | [DescribeAlarms](https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_DescribeAlarms.html) |
| `logs` | CloudWatch Log Groups | `retentionInDays` is nil → Warning (Never Expire = cost drift). `storedBytes==0 && creationTime<now()-90d` → Warning (orphan). `kmsKeyId` empty → Warning (not encrypted with KMS). Cross-ref `kms` — referenced `kmsKeyId` is in `PendingDeletion` → Broken — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | `DescribeMetricFilters` per CloudTrail (`/aws/cloudtrail/`-prefixed) log group: no metric filters → Warning (`audit log group missing metric filters`). `DescribeLogStreams(orderBy=LastEventTime, descending=true, limit=1)` per log group feeds the `last_event_at` column only; `lastEventTimestamp` stale beyond expected write cadence → Warning (silent service) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | None | [DescribeLogStreams](https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_DescribeLogStreams.html) |
| `trail` | CloudTrail Trails | `LogFileValidationEnabled==false` → Warning (field is on `DescribeTrails` list response); `CloudWatchLogsLogGroupArn` empty → Warning (delivering to the bucket only); `KmsKeyId` empty → Warning (log files not KMS-encrypted) | `GetTrailStatus` per trail: `IsLogging==false` → Broken (trail stopped capturing); `LatestDeliveryError` non-empty → Broken (S3 delivery failing); `LatestDeliveryTime` >1h ago on `IsLogging==true` trail → Broken. `GetBucketPolicyStatus`/`GetBucketLogging` on the trail's own log bucket: public → Broken; no access logging → Warning | `LookupEvents` absence detection | [GetTrailStatus](https://docs.aws.amazon.com/awscloudtrail/latest/APIReference/API_GetTrailStatus.html) |
...
| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `cfn` | CloudFormation Stacks | `StackStatus`: `CREATE_COMPLETE`/`UPDATE_COMPLETE`/`IMPORT_COMPLETE`/`UPDATE_COMPLETE_CLEANUP_IN_PROGRESS`→Healthy; `*_IN_PROGRESS`/`REVIEW_IN_PROGRESS`→Warning; `ROLLBACK_COMPLETE`→Warning (failed-create tombstone — stack operationally dead, delete-and-recreate required, but not actively failing); `UPDATE_ROLLBACK_COMPLETE`/`IMPORT_ROLLBACK_COMPLETE`→Warning (update failed, stack reverted to prior state); `*_FAILED`→Broken. Status phrases render as words, not the raw status. Posture: `enableTerminationProtection` absent or false on a live top-level stack (nested stacks inherit the root's setting and are skipped) → Warning; an `outputs[]` value matching the credential scanner → Broken, with the output key and match kind on the row and never the value. `IN_PROGRESS` >1h → Broken (stuck) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | `DescribeStackEvents` per stack — take the first response page and scan the most recent events client-side: recent event `ResourceStatus==*_FAILED` → Broken. `DescribeStacks` per stack: `DriftInformation.StackDriftStatus==DRIFTED` → Warning (signal is low-coverage until `DetectStackDrift` has been run) | `DetectStackDrift` + `DescribeStackDriftDetectionStatus` (async polling) for fresh drift detection | [DescribeStacks](https://docs.aws.amazon.com/AWSCloudFormation/latest/APIReference/API_DescribeStacks.html) |
...
| `cb` | CodeBuild Projects | Posture, from the `BatchGetProjects` the fetcher already makes: `projectVisibility==PUBLIC_READ` → Broken; a buildspec that is a repository path or is empty on a source type a contributor can write to → Warning; userinfo in `source.location` → Broken (the row shows the address with the credential removed); a `PLAINTEXT` environment variable matching the credential scanner → Broken (Parameter Store and Secrets Manager variables are references and are skipped) | Latest build status per project (via `ListBuildsForProject(maxResults=1)` + batched `BatchGetBuilds`): latest `buildStatus` in `FAILED`/`FAULT`/`TIMED_OUT` → Broken (excluding user-initiated `STOPPED`) | Stale-project (>90d); cache-config + perf signals | [BatchGetBuilds](https://docs.aws.amazon.com/codebuild/latest/APIReference/API_BatchGetBuilds.html) |
| `ecr` | ECR Repositories | `imageScanningConfiguration.scanOnPush` absent or false → Warning; `imageTagMutability==MUTABLE` → Warning (`IMMUTABLE_WITH_EXCLUSION` still pins the tags that matter and is not the signal) | One `DescribeImages(maxResults=10)` call per repository (AWS returns the most recent images; `imageScanFindingsSummary` arrives inline): aggregated across the sampled images, `findingSeverityCounts.CRITICAL>0` → Broken, else `HIGH>0` → Warning — one `ecr.vulnerabilities` finding (summary present only when scan has run). `GetRepositoryPolicy` per repository through the shared policy engine: a wildcard principal with no restrictive condition → Broken. `GetLifecyclePolicy` per repository: the NotFound answer is itself the finding → Warning | `DescribeImageScanFindings` per image | [DescribeImages](https://docs.aws.amazon.com/AmazonECR/latest/APIReference/API_DescribeImages.html) |
...
| shortName | Name | Wave 1 | Wave 2 | Wave 3 | Source |
|---|---|---|---|---|---|
| `glue` | Glue Jobs | Posture, from the `GetJobs` response itself: no `securityConfiguration` → Warning (one signal covering the S3, CloudWatch-logs and bookmark encryption checks, which all live on that configuration); `defaultArguments` without `--enable-continuous-cloudwatch-log` set to `true` → Warning; a `defaultArguments` value matching the credential scanner → Broken (the leading `--` is stripped before scanning) | `GetJobRuns(maxResults=1)` per job (API returns runs in descending-by-start-time order): latest `JobRunState` in `FAILED`/`TIMEOUT`/`ERROR`/`EXPIRED` → Broken (excluding user-initiated `STOPPED`) | DPU-hours trend; bookmark-stuck detection | [GetJobRuns](https://docs.aws.amazon.com/glue/latest/webapi/API_GetJobRuns.html) |
| `athena` | Athena Workgroups | `State`: `ENABLED`→Healthy; `DISABLED`→Warning (admin off) | `GetWorkGroup` per workgroup: `EnforceWorkGroupConfiguration==false` → Warning (`athena.settings-not-enforced`); `ResultConfiguration.EncryptionConfiguration==nil` → Warning (`athena.results-unencrypted`) — two independent findings, since a workgroup can fail either without the other. | `ListQueryExecutions`+`BatchGetQueryExecution` failure-rate per workgroup | [GetWorkGroup](https://docs.aws.amazon.com/athena/latest/APIReference/API_GetWorkGroup.html) |
| `mwaa` | Managed Airflow | None — `ListEnvironments` returns environment name strings only | `GetEnvironment` per environment (N+1; accounts run 1–5 envs): `Status`: `AVAILABLE`→Healthy; `CREATING`/`CREATING_SNAPSHOT`/`PENDING`/`UPDATING`/`ROLLING_BACK`/`MAINTENANCE`→Warning; `CREATE_FAILED`/`UPDATE_FAILED`/`UNAVAILABLE`→Broken; `DELETING`/`DELETED`→Dim. `LastUpdate.Status==FAILED` on an `AVAILABLE` environment → Warning (last update failed; surface `LastUpdate.Error.ErrorMessage`). `WebserverAccessMode` in `PUBLIC_ONLY`/`PUBLIC_AND_PRIVATE` → Warning (webserver reachable from the internet). AccessDenied on `mwaa:ListEnvironments` → menu row shows the error, never `0`. AccessDenied on `airflow:GetEnvironment` for a listed name → the row is KEPT (name-only) with `details denied` → Warning, plus the aggregated fetch error — a denied describe must never make a listed environment vanish | CloudWatch `AWS/MWAA` metrics: `SchedulerHeartbeat`, DAG-processing `ImportErrors`/`TotalParseTime`, `QueuedTasks`/`RunningTasks`, worker/scheduler CPU+memory. AirflowVersion-EOL check (no stable API source) | [GetEnvironment](https://docs.aws.amazon.com/mwaa/latest/API/API_GetEnvironment.html) |
...
| ec2 | ec2.imdsv1-allowed | IMDSv1 allowed | warn | wave1 |
| ec2 | ec2.public-ip | public address | warn | wave1 |
| ec2 | ec2.internet-exposed | port(s) <list> reachable from the internet | broken | wave2 |
| ec2 | ec2.user-data-secret | credential in user data | broken | wave2 |
| ecs-svc | ecs-svc.state.inactive | inactive | broken | wave1 |
| ecs-svc | ecs-svc.state.draining | draining | warn | wave1 |
| ecs-svc | ecs-svc.tasks.none-running | no tasks running | broken | wave1 |
| ecs-svc | ecs-svc.tasks.below-desired | running below desired count | warn | wave1 |
| ecs-svc | ecs-svc.deployment-failed | deployment failed | broken | wave2 |
| ecs-svc | ecs-svc.public-ip | tasks get public IPs | warn | wave2 |
...
| ecs-task | ecs-task.privileged | privileged container | broken | wave2 |
| ecs-task | ecs-task.host-namespace | shares the host network or process namespace | warn | wave2 |
| ecs-task | ecs-task.writable-root | writable root filesystem | warn | wave2 |
| ecs-task | ecs-task.no-logging | container without log driver | warn | wave2 |
| ecs-task | ecs-task.env-secret | credential in container environment | broken | wave2 |
...
| lambda | lambda.env-secret | credential in environment variables | broken | wave1 |
| lambda | lambda.public-policy | invokable by anyone | broken | wave2 |
| lambda | lambda.function-url-public | function endpoint open without authentication | broken | wave2 |
...
| asg | asg.launch-config.legacy | uses a launch configuration | warn | wave1 |
| asg | asg.single-az | single availability zone | warn | wave1 |
| asg | asg.no-elb-health-check | no load balancer health check | warn | wave1 |
| asg | asg.launch-config.imdsv1 | launch configuration allows IMDSv1 | warn | wave2 |
| asg | asg.launch-config.public-ip | launch configuration assigns public IPs | warn | wave2 |
| asg | asg.launch-config.secret | credential in launch configuration user data | broken | wave2 |
...
| ebs | ebs.not-in-backup-plan | not covered by a backup plan | warn | wave2 |
| ebs | ebs.no-snapshot | no snapshot exists | warn | wave2 |
...
| ebs-snap | ebs-snap.public | shared with all AWS accounts | broken | wave2 |
...
| ami | ami.public | shared with all AWS accounts | broken | wave1 |
...
| lt | lt.user-data-secret | credential in user data | broken | wave2 |
...
| sg | sg.default-with-rules | default group allows traffic | warn | wave1 |
| sg | sg.unused | not attached to anything | warn | wave2 |
...
| subnet | subnet.auto-public-ip | auto-assigns public IPs | warn | wave1 |
...
| tgw | tgw.auto-accept-attachments | auto-accepts shared attachments | warn | wave1 |
...
| dbi | dbi.not-in-backup-plan | not covered by a backup plan | warn | wave2 |
...
| dbc | dbc.not-in-backup-plan | not covered by a backup plan | warn | wave2 |
...
| ddb | ddb.not-in-backup-plan | not covered by a backup plan | warn | wave2 |
...
| opensearch | opensearch.public | reachable outside a VPC | broken | wave1 |
| opensearch | opensearch.https-not-enforced | HTTPS not enforced | warn | wave1 |
| opensearch | opensearch.node-to-node-tls-off | node-to-node encryption off | warn | wave1 |
...
| dbi-snap | dbi-snap.warn.transitional | <status> | warn | wave1 |
...
| dbc-snap | dbc-snap.warn.transitional | <status> | warn | wave1 |
...
| alarm | alarm.actions-disabled | actions disabled | warn | wave1 |
...
| logs | logs.no-kms | not encrypted with KMS | warn | wave1 |
...
| trail | trail.no-cloudwatch-logs | not delivering to CloudWatch Logs | warn | wave1 |
| trail | trail.no-kms | log files not KMS-encrypted | warn | wave1 |
| trail | trail.log-bucket-public | log bucket is publicly accessible | broken | wave2 |
| trail | trail.log-bucket-no-access-logging | log bucket has no access logging | warn | wave2 |
...
| sqs | sqs.no-kms | not encrypted with KMS | warn | wave2 |
...
| secrets | secrets.public-policy | resource policy open to anyone | broken | wave2 |
| secrets | secrets.cross-account-policy | resource policy grants another account | warn | wave2 |
...
| kms | kms.public-policy | key policy open to anyone | broken | wave2 |
...
| r53 | r53.query-logging-off | query logging off | warn | wave2 |
| r53 | r53.dangling-record | record points at a released address | broken | wave2 |
| cf | cf.insecure-protocol | no HTTPS redirect (insecure); origin without TLS | warn | wave2 |
| cf | cf.origin-bucket-missing | S3 origin bucket does not exist | broken | wave2 |
| cf | cf.deprecated-tls | minimum TLS below 1.2 | warn | wave2 |
| cf | cf.logging-off | access logging off | warn | wave2 |
| cf | cf.no-default-root-object | no default root object | warn | wave2 |
| cf | cf.s3-origin-no-oac | S3 origin without origin access control | warn | wave2 |
| cf | cf.default-certificate | uses the default CloudFront certificate | warn | wave2 |
| cf | cf.no-geo-restriction | no geo restriction | warn | wave2 |
...
| acm | acm.weak-key | weak key algorithm | warn | wave1 |
...
| apigw | apigw.no-authorizer-public | internet-facing with no authorizer | broken | wave2 |
| apigw | apigw.no-authorizer | no authorizer | warn | wave2 |
| apigw | apigw.no-access-logs | no access logs | warn | wave2 |
| apigw | apigw.tracing-off | X-Ray tracing off | warn | wave2 |
| apigw | apigw.stage-variable-secret | credential in stage variables | broken | wave2 |
...
| iam-user | iam-user.console-dormant | console sign-in unused for 90 days | warn | wave2 |
...
| cfn | cfn.stack.failed | <status, in words> | broken | wave1 |
| cfn | cfn.stack.rollback | <status, in words> | broken | wave1 |
| cfn | cfn.stack.in\_progress | <status, in words> | warn | wave1 |
| cfn | cfn.stack.deleted | delete complete | dim | wave1 |
