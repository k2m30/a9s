...
| `athena` | Athena Workgroups | `State`: `ENABLED`→Healthy; `DISABLED`→Warning (admin off) | `GetWorkGroup` per workgroup: `EnforceWorkGroupConfiguration==false` → Warning; `ResultConfiguration.EncryptionConfiguration==nil` → Warning — each an independent row of one `athena.governance-misconfigured` finding. `BytesScannedCutoffPerQuery` unset → Warning (cost) — NOT IMPLEMENTED (backlog; no emission in code as of 2026-07-19) | `ListQueryExecutions`+`BatchGetQueryExecution` failure-rate per workgroup | [GetWorkGroup](https://docs.aws.amazon.com/athena/latest/APIReference/API_GetWorkGroup.html) |
