...
- **How discovered**: cross-reference the already-loaded `rtb` list for live `Routes[].VpcPeeringConnectionId == <pcx-id>`. AWS leaves the id on the route and marks it `blackhole` once the connection is gone, so a blackhole route names no path to the peer and is not counted. Zero extra API calls. Unknown (`?`) when the rtb cache is absent; a truncated cache renders the count as an honest lower bound (`N+`, the fleet cache-scan convention shared with lt/ng) — never a fake exact 0.
- **Count shown**: yes.
