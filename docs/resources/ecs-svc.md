...
- **How discovered**: the rules whose `EventPattern` matches an event ECS emits about this service — "ECS Service Action" (`resources` `[<service ARN>]`, `detail.clusterArn`) and "ECS Task State Change" (`detail.group` `service:<name>`, `detail.clusterArn`) ([ecs_service_events](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_service_events.html), [ecs_task_events](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_task_events.html)); the pattern is read the way EventBridge reads one — literal lists and every content filter it documents (`prefix`, `suffix`, `anything-but`, `equals-ignore-case`, `exists`, `numeric`, `cidr`, `wildcard`, `""`, `null`, `$or`) ([eb-event-patterns](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-event-patterns.html), [eb-create-pattern-operators](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-create-pattern-operators.html)); a pattern it cannot read (an undocumented operator, a malformed filter), a field the events do not carry, or one naming no resource (the source alone) makes the count a lower bound rather than a match. Plus the rules that target the service's cluster (`ListRuleNamesByTarget` on the cluster ARN) with `EcsParameters.TaskDefinitionArn` of the service's task-definition family (`ListTargetsByRule`), whatever their pattern.
- **Count shown**: yes.
...
- **How discovered**: resolve `Service.TaskDefinition` → `DescribeTaskDefinition` → `ContainerDefinitions[].LogConfiguration` read each container's group: `Options["awslogs-group"]` for the `awslogs` driver, and for `awsfirelens` the Fluent Bit CloudWatch output's `log_group_name` ([firelens-taskdef](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/firelens-taskdef.html)), a config file or `log_group_template` leaving a lower bound; cross-reference the already-loaded `logs` list by `logGroupName`. A definition that cannot be read (the call refused, or no ECS client) proves nothing either way, so the row falls back to the log groups whose name carries the family, as candidates without a count — a9s-devops-persona: awslogs is the overwhelmingly common driver for ECS; `Options["awslogs-group"]` names the log group directly.
- **Count shown**: yes.
- **Service log view** (`L`): the newest 200 lines of the service, newest first. Every awslogs container of the task definition is read — its `awslogs-group`, narrowed to its own streams by `LogStreamNamePrefix` = `<awslogs-stream-prefix>/<container name>/` (the awslogs stream name is `<prefix>/<container>/<task-id>`), in its `awslogs-region` through the session's client for that Region — and the containers' lines are merged newest first. Each source is read back from now in windows of doubling width (`FilterLogEvents` answers oldest-first, so each window is read whole). The oldest line shown is newer than every line not shown; Load More continues with the next older 200. A window that does not fit the bounded read (100 calls per source) is left unread and the view is marked partial; Load More retries it narrowed to a quarter of its width, so every press makes progress. A single millisecond that alone does not fit shows what was read of it, is marked partial, and Load More continues it from the `FilterLogEvents` `nextToken` where the read stopped. Rows are keyed by `eventId`. A line read in another Region carries that Region, and its console link (`o`) opens that Region's console.
...
- **How discovered**: `Service.RoleArn`, and the task definition's `taskRoleArn` and `executionRoleArn` ([API_TaskDefinition](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_TaskDefinition.html)).
- **Count shown**: yes.
...
- **How discovered**: `DescribeStateMachine` for each loaded state machine; a Task state whose `Resource` is the `ecs:runTask` integration runs the service's task-definition family when its `TaskDefinition` — in `Parameters` (JSONPath) or `Arguments` (JSONata), as a family, `family:revision` or an ARN — has that family ([connect-ecs](https://docs.aws.amazon.com/step-functions/latest/dg/connect-ecs.html)). A family that is a prefix of another is a different family. A `TaskDefinition` given by a path or a JSONata expression cannot be read and leaves the count a lower bound.
