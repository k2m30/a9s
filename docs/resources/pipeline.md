...
|---|---|---|---|---|---|
| `latestExecution.status == Failed` | 2 | Broken | `!` | S1, S2, S3, S4, S5 | `stage failed` |
...
pipeline — CI/CD. Status key: `last_status` — the key the status cell reads, and the column naming it is the status column.
