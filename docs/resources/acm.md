...
- **List API**: `ListCertificates` — returns `CertificateSummary[]`. The SDK confirms `Status`, `NotAfter`, `NotBefore`, `InUse`, `DomainName`, `CertificateArn` are all on the summary shape, so every Wave 1 signal is reachable with zero extra calls. The call names every `KeyAlgorithm` in `Includes.KeyTypes` and every `CertificateKeyPairOrigin` in `CertificateKeyPairOrigins`: unfiltered, it leaves out most key types and every ACME-issued certificate.
