...
**Cache-scan enricher** core/aws/vpcpeer_issue_enrichment.go (EnrichLTDeprecatedAMI/snapshot_cross_ref.go layer, zero SDK calls, scans the "rtb" cache):
...
**Related checkers** core/aws/vpcpeer_related.go:
...
## 2. Fixture list (core/demo/fixtures/vpcpeer.go; synthetic account 123456789012 + remote 210987654321)
...
| File | Owner |
|---|---|
| core/demo/fixtures/vpcpeer.go + rtb/vpc sibling route entries | 6a coder |
| core/demo/fakes/ec2.go — DescribeVpcPeeringConnections handler | 6a coder |
| fixtures/counts.go "vpc-peer" entry | 6a coder |
| core/aws/vpcpeer.go — fetcher + CIDR overlap math | 7 coder |
| core/aws/vpcpeer_issue_enrichment.go — rtb cache-scan | 7 coder |
| core/aws/vpcpeer_related.go — rtb/vpc/ct-events | 7 coder |
| core/aws/lt_interfaces.go-style narrow iface if needed (EC2API likely already carries the op — check ec2_interfaces.go first) | 7 coder |
| core/aws/catalog_networking.go — ResourceTypeDef (Aliases [vpc-peer pcx peering] — uniqueness gate) | 7 coder |
| core/config/defaults_networking.go — columns: Pcx Id, Status, Requester VPC, Requester Owner, Accepter VPC, Accepter Owner, Expires | 7 coder |
