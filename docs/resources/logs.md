...
- **How discovered**: Call `logs:DescribeMetricFilters(logGroupName=…)` to list the filters attached to this log group, read each filter's `metricTransformations[].metricName` + `metricNamespace`, then scan the already-loaded `alarm` list for the alarms over one of those metrics, matching on `MetricName` + `Namespace`. Alongside that, an alarm in namespace `AWS/Logs` carrying a `LogGroupName` dimension with this group's name is an alarm on the group's own volume metrics. — a9s-devops: the log group ↔ alarm linkage is indirect (log group → metric filter → CloudWatch metric → alarm); no direct field bridges them. possible=yes (DescribeMetricFilters is read-only, per-group, one call per open), worth=yes (this is the canonical "what alerts if this log group goes bad?" workflow). The alarm end reads the same filters from the other side — `metricName` + `metricNamespace` — so the two directions agree.
- **Count shown**: yes.
