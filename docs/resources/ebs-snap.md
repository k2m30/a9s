...
- **How discovered**: no direct field on `Snapshot` points at a Backup plan. AWS Backup-created snapshots carry a `Description` beginning `"Created by AWS Backup ..."` and the auto-tag `aws:backup:source-resource`, whose value is the source volume's ARN. That ARN is evaluated against the already-loaded `backup` list: a plan counts iff any one of its selections does, each applied as AWS does: `(Resources match OR ListOfTags match) AND every Conditions clause AND NOT NotResources match`, with AWS wildcard semantics. A match on `*`, a service name (`arn:aws:<service>:*`) or tags alone counts only when the Region has the resource's type opted in to AWS Backup (`DescribeRegionSettings`, read with the backup list); a pattern naming the resource type or the exact ARN counts regardless. When the opt-in could not be read and a match rests on it, the count is unknown. A snapshot with the Description alone and no source tag shows the count as unknown.
- **Count shown**: unknown — `docs/related-resources.md` § `ebs-snap` does not specify.
