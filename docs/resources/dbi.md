...
- **Signal**: no backup plan selection matches this instance. An instance in a cluster (Aurora, DocumentDB, Neptune or a Multi-AZ DB cluster) is judged by its cluster's ARN, tags and engine, by the rule in dbc.md §3: AWS Backup protects it through the cluster, and `arn:aws:rds:*:*:db:*` selects no Aurora, DocumentDB or Neptune instance. For any other instance, a match on `*`, a service name (`arn:aws:<service>:*`) or tags alone counts only when the Region has the resource's type opted in to AWS Backup (`DescribeRegionSettings`, read with the backup list); a pattern naming the resource type or the exact ARN counts regardless. When the opt-in could not be read and a match rests on it, the count is unknown. A row the check cannot decide — a plan whose selections were not all read, tags or the Region opt-in that were not read, a selection tag clause returned without its value — is marked not inspected, naming the check that stopped it.
  - **State bucket**: Warning.
  - **How obtained**: read on the type's bounded Wave 2 pass, which the catalog registers for this type.
