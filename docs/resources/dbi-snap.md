...
- **Describe API (if any)**: not used — all Wave 1 signals are carried on the list response; Wave 2 is `None` per `attention-signals.md § Signals § DATABASES & STORAGE` row `dbi-snap`.
...
- Manual-snapshot cost-drift age rule (> 365d on `SnapshotType=="manual"`) — not present in `attention-signals.md § Signals § DATABASES & STORAGE` row `dbi-snap` for dbi-snap (it applies to `dbc-snap`); out of scope here until the golden doc adds it.
...
- a9s golden doc — Wave 1 signals (`Status` buckets, `Encrypted==false`, orphan cross-ref `dbi`, automated-past-retention cross-ref `dbi`) — `docs/attention-signals.md § Signals § DATABASES & STORAGE` row `dbi-snap` (Wave 1 cell).
- a9s golden doc — Wave 2 cell is `None` — `docs/attention-signals.md § Signals § DATABASES & STORAGE` row `dbi-snap` (Wave 2 cell).
- a9s golden doc — Wave 3 exclusion (`DescribeDBSnapshotAttributes` per snapshot, public-snapshot) — `docs/attention-signals.md § Signals § DATABASES & STORAGE` row `dbi-snap` (Wave 3 cell).
