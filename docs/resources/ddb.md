...
- **How discovered**: Reverse-scan the already-loaded `backup` list selections (`GetBackupSelection.Resources[]`) for `arn:aws:dynamodb:<region>:<account>:table/<name>` matching this table's ARN, or the resource-type entry `arn:aws:dynamodb:*:*:table/*` combined with tag-based inclusion. — a9s-devops: Backup coverage lives on the plan's selection, not on the table; reverse-scan against the already-loaded `backup` list is the cheapest approach. AWS Backup's `ListRecoveryPointsByResource(ResourceArn=<table ARN>)` is a per-table Wave 2 call and is out of scope for the panel.
- **Count shown**: yes.
...
- a9s-devops consultation — `backup` discovery via `GetBackupSelection.Resources[]` reverse-scan — a9s-devops persona (2026-04-20): possible=yes, worth=yes. Backup coverage lives on the plan's selection, not on the table; the table itself has no field pointing at its backup plans.
