...
| ec2.state.shutting-down | shutting down | warn | wave1 | The instance is being terminated and the call cannot be taken back. Anything on its instance-store volumes is gone; an EBS volume survives only if it was set not to delete on termination. If this was not intended, find the snapshots and retained volumes and launch a replacement from them. |
