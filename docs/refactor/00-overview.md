...
4. **No misleading permanent API.** If a PR introduces a new exported API, function signature, or struct field, that API MUST be the intended end-state shape — not a transitional shape that a later PR will rename. Renaming carries a churn cost the program cannot afford to pay twice.
