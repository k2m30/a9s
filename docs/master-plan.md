...
**Recommendation:** VHS with a `.tape` file checked into `docs/demos/demo.tape`. Use the `dev-account` AWS profile for real data. Generate both GIF (for README) and MP4 (for website).
...
### Phase 2: Demo & README
7. Record demo with VHS (needs `dev-account` AWS profile)
