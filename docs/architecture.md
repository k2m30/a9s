...
- **`core/session`** — session-scoped state container (Phase 02 deliverable, **done**): `session.Session` owns per-profile/region orchestration state — the `RowStore` (the single session-scoped per-type row store; see Caching Layers), `RelatedCache`, the enrichment queues and per-type maps, every generation counter (all typed `domain.Gen` after Phase 05a-gens), and the capability stores (`PolicyStore`, `IdentityStore`, `RuleSetStore`, and `RegionListStore` for another Region's lists, all listed in `core/session/stores.go`) — the first three replaced the deleted `core/aws/` package globals (`allPoliciesMu`, `identityCacheMu`, `sesRuleSetCacheMu`). `Session.Rotate()` is the single point that invalidates all of it on profile/region switch.
...
7. **Feature-specific caches do not hang off transport objects.**
   ...
   Region is read once per session through `RegionListStore`
   (`core/aws/region_list_store.go`), a session store every rotate and
   refresh replaces. The session stores the AWS transport reads through are
   listed once, in `core/session/stores.go`: `New`, `Rotate`, `WireStores`
   and `RefreshStores` walk that list, so a new store is reset, wired and
   refreshed by adding one entry. A resource's identity is its
...
9. **AWS-supplied text is made inert where it enters, never where it is
   ...
   is what the absorb-latency pin forbids), while the writer it protects runs
   under it. Every door into controller state that accepts a
   `resource.Resource`, a `[]resource.Resource`, findings or AWS error text
   calls one of them: `Controller.Handle` and
   `Controller.HandleResourcesLoadedEvent` — the TUI's own door — for a
   loaded page (`SanitizedRows`), the `ApplyResourcesLoaded` test seam, the
   cache-replay seed in `applyNavResult`, `ensureDetailState` for a resource
   opened straight into a detail screen,
...
| `RelatedCheckResultMsg` | Deliver one related-check result to detail view — carries `Result.Err` (checker failure), `LazyAddError` (FetchByIDs failure; a panicking checker is recovered by `RunRelatedDef` in `core/runtime/executor.go` and surfaced as that row's error), `LazyAddedResources` (out-of-scope targets resolved via `FetchByIDs`), `CachedPages` (cold-miss prefetch); app handler routes errors to `FlashMsg{IsError:true}` so the `!` error log captures them |
...
core/            # platform-agnostic core — renderer-free, gated by `make verify-renderer-free`
  ...
  config/        # YAML config loading, built-in DETAIL fields per service category (defaults_<category>.go); the list columns are read off the catalog
...
type ResourceTypeDef struct {
    ...
    Columns     []domain.Column  // table columns for list view — Key/Path (what the cell reads), Title, Width, SortKey
    LifecycleKey string          // Fields key holding lifecycle state, and the type's declaration of WHICH column is its status column: the one naming this key. Defaults to "state"
...
`Columns []domain.Column` is the ONE declaration of a type's list columns: which there are, in what order, each with its title, its width, where its cell reads its value (`Key`, `Path`) and what it sorts on (`SortKey`). Every column says where its value comes from: a column that declared neither had its cell looked up by turning the title into a field key, which held only while the title and the fetcher's key agreed letter for letter. The fifteen status columns that were resolved that way name `state`, the lifecycle key their cell reads — their value is the type's finding phrase and no fetcher writes it, which the producers gate answers with one rule about the shape rather than an entry per type.

`config.DefaultConfig()` derives the built-in view's `list:` block from the catalog — `config.ListColumnsFromCatalog` — so there is no per-type column list in `core/config` to drift from it, and `go run ./cmd/viewsgen/` writes what the catalog says into `.a9s/views/*.yaml`.

It was two declarations, and they disagreed. The defaults declared columns the catalog did not (a Status column on twenty-nine types), at different widths, in a different order, and the catalog declared columns no view ever rendered (six on `ct-events`); which of the two a change reached depended on which file its author happened to open, which is how lambda's Handler column existed on the demo bench and on nobody's account. A `Sortable` flag rode along on every column until nothing was found to read it: `handleActionSort` sorts on whatever key the action carries, so a column declaring itself unsortable sorted like any other, and the tests asserting the flag were asserting the literal back to itself. It is gone; a column that must refuse a sort needs the refusal first.

**The status column is declared, not inferred.** A type's `LifecycleKey` names the Fields key its state lives under — read everywhere through `ResourceTypeDef.StatusKey()`, which supplies the `state` default the four callers used to each write out — and the column naming that key IS its status column — `config.IsStatusColumn` is the one answer, asked by the render cascade, the decorator lookup and the status-column resolver, all in `core/app/list_columns.go`; the cache save lane needs no predicate, since with one key the status column is the keyed column it is. A type whose state is a computed verdict rather than an API field names that verdict's key: tg's `health_summary`, sg's `risk_summary`, cb's `last_build`.

An operator's own file is where the two used to be told apart by accident: a column was the status column if its title said Status, or if its key said `status` whatever the type calls its own key, so a renamed column with a hand-typed `key: status` worked. Neither is true now, so the migration owns those keys like any other it moved. A migration encodes the rule it replaced: at stamp 7 `movedStatusKey` is the OLD predicate written out — a key of `status`, or a title of Status or State whatever the key said — and every column it recognises takes the key the type now declares. It resolves the type with `catalog.FindAny`, the one resolver for a view name, because a child type has a view file and an operator too, and looking only among the parents stamped every child file as migrated while migrating nothing.

"Untouched" is equality with what the previous version generated, never "differs from what this build declares". The v6 key move only ever ADDED keys — 325 of them, and not one column's key was replaced — so a column is the previous generator's work when it carries NO key where this build declares one, and an operator's own key is theirs. Asking merely whether the key differed read their key as the generator's output, and since the same question decides whether the file was ever edited at all, one such key made the whole file look untouched and be replaced wholesale.

`catalog.FindAny` is the only exported resolver for a type name. `find` and `findChild` are unexported, because a lookup that answers for half the catalog reads like a general one at the call site and every caller that picked a half was wrong for the other: a child view file was "no such type", and a child type's `Related` and `Navigable` were declarations nothing read. `TopLevelOnly` and `ChildOnly` exist for the three callers whose answer IS one half — `resource.FindResourceType` is the parents accessor, `GetChildType` the children one, `GetPaginatedChildFetcher` reads a field only a child has — and their names say so where `Find` never did. Inside `core/resource` the getters ask `typeDef`, which is `GetChildType` (so a type a test registered is a type) then `FindAny`.

What a load can say about a file it read is one report. A file whose name resolves to no type this build has is a file the operator wrote that reaches no screen, so the loader keys every view by the RESOLVED short name — `EC2.yaml` and an alias both land on `ec2` — and reports a name that resolves to nothing. A column nothing can fill is the same mistake one line lower: `config.ColumnFilled` is the one rule for it, asked by the report and by the producers gate, and it counts a RawStruct path as the producer it is — the cascade reads the struct live and `MaterializeListFields` writes that value under the column's key for the next start. A file under a name this build has RENAMED is the generator's own leftover, not the operator's mistake: `viewFileRenames` (`core/config/ensure_views.go`) carries the repo's two renames — docdb-snap to dbc-snap, rds-snap to dbi-snap — and `EnsureViewsDir` moves such a file to the name in use, or sets it aside under a suffix the loader does not read when that name already has a file, which is the one being edited. Either way it is one debug-log line and no flash. Add a row in the same change that renames a type whose view file this repo carries; the gate reads the rename history out of git, so a rename without a row is red.

Two files in ONE directory naming one type is the third: the later one replaces the earlier WHOLE — merging them per field would put one file's columns beside the other's detail rows, a screen neither file describes — and the report names both and says which is in use. Across directories it is not a collision but the documented lookup chain. `config.LoadFromDirs` reports all three, once each, and hands back the config anyway. Both lanes keep the file and show the report in the same words: `config.ReportText` composes the one sentence — which says whether the operator's file or the built-in defaults are in use, because a parse error means nothing loaded while an unfillable key costs them one column — and the TUI flashes it, the web page carries it in the flash its own header renders, and the web server logs a copy to stderr. The web used to write it to stderr alone, after replacing the whole config with the defaults.

The cell reads a finding phrase first — the phrase is what the operator is being told about the row — and then that one key, and then the column's `Path`. It used to read the lifecycle key, then the literal `"status"`, then the column's own key, while the predicate answered on the TITLE as well, so a column titled Status was the status column whatever it declared and the declaration was consulted third: tg showed `available` where its own column said `unhealthy targets: 2/5`, and cb showed `succeeded` for `last build failed`. The save lane carried its own copy of that three-spelling order so the key it persisted under matched the key the replay read first; with one key there is nothing to copy, and `saveFieldKey` treats the status column as the keyed column it is — which is what makes the cell survive a restart unchanged.

`resource.ResolveListColumnCascade` is still the one resolver, because an operator's own file is a third thing: it owns which columns there are, their order and their widths, and the catalog's `Key` is merged onto every title both declare, through `mergeListColumn`. A type no view declares at all — a type a test registers is the only one left — resolves through `config.ListColumnsFromCatalog`, the same translation the built-in views come from. It used to build those columns by hand instead, and dropped the path and the sort key while doing it, so a type registered by a test resolved its columns by different rules from one the catalog registers, and a test could pass on a shape production never renders.

It was two paths, and that was a defect rather than a subtlety. A loaded view file came back verbatim, so the catalog's `Key` was dropped for anyone who had ever started a9s, while a nil config got the merged set. The demo bench and the account an operator looks at therefore resolved cells by different rules: a cell could be right on the bench that acceptance reviews and wrong on every real screen, which is what CodeBuild's Source Type was, and lambda's Handler column existed on the bench and on nobody's account. `tests/unit/misc4_r8_columns_agree_test.go` holds the overlap honest: for every title both declare, the two arms must render the same cell over every demo row, and hold one order.

Whether a field's value is an AWS constant an operator should not have to read is declared on neither of those two: it is `ResourceTypeDef.HumanizeFields`, a list of field identifiers on the type itself. The list column, the detail projector and any later reader resolve it through `catalog.HumanizeFieldKey`, which drops case and the separators between words — underscores, spaces and dots — so one entry answers every spelling the same fact is reached by — a RawStruct path on a column, a dotted path on a nested one, a snake_case key from a fetcher, a label on a detail row. The dot is load-bearing rather than tidy: one fact reaches its surfaces under different spellings — a column names it by the RawStruct `Path` the view declares, a fetcher by a snake_case key, and the detail projector by the composed path a nested row renders under — and an entry that did not fold the dot would answer for one surface while the other went on printing the constant. A path whose leaf is the fact but whose ancestors are not (`StreamModeDetails.StreamMode`) does not fold to its leaf — dropping a word is not normalizing a spelling — and is declared alongside the leaf's own. The declaration belongs to the FACT and not to a surface: while it lived on the column, a field no column happened to show had nothing to carry it, and the detail rendered the SDK constant verbatim (mwaa's `EndpointManagement`), while a field with a column could read as words in the list and as a constant in the detail. Only enum-shaped values belong there — a name AWS assigned and a person types back stops being searchable once it is reworded. `tests/unit/aws6_humanize_owner_test.go` sweeps every demo detail row for a value shaped like an SDK constant. Its survivors are two lists that are not the same thing. `verbatimDetailPaths` carries the reason each value is an identifier a person types back, and every reason has to name the AWS field whose value that is — in the field's own words or by spelling its name — because a reason that says only what the value is used for does not say which field must not be reworded; the list also carries a recorded size, so an entry is added by a line a reviewer reads rather than by one more key in a long map. `rawEnumDetailDebt` was w198's worklist and is empty, under a ceiling of zero: an entry above the ceiling fails, an entry that stops rendering raw fails until the ceiling comes down with it, and a constant on neither list fails the sweep, so the list can only shrink.

A9s's own verdicts are not declared at all: a classification a9s computes rather than reads from AWS — a log line's class, a role's trust summary — is written in the words it renders in, at the one place that computes it.
...
**Paging** (`core/aws/pager.go`): every paged read outside a list fetcher goes through `PageAll(ctx, maxPages, next)`, which retries each page through `RetryOnThrottle` and returns `(items, complete, err)`: the items of every page read, `complete` false when it stopped at its cap with a token pending or failed, and on error the pages read before the failure. A caller never retries inside `next`. A read of one page on purpose goes through `firstPage`, and only for a request that names its one item, asks whether a page exists, or an API whose reference documents an order that puts the answer on the first page. An account-wide enricher walk uses `walkAccountPages`, built on `PageAll`. A list fetcher hands its cursor back to the list view instead. EC2 describes by ID go through `fetchEC2ByIDs` (`core/aws/ec2_by_ids.go`): at most 1,000 IDs per call, and the IDs a NotFound error names are dropped and the batch asked again. Guard tests fail on a paged read outside these, a retry inside a pager, a `firstPage` read of an API with no documented order, and an EC2 by-ID fetch outside the owner.

**Service clients** (`core/aws/client.go`): a session may hold a client set without a given service's client, so a `ServiceClients` field is nil until checked. `serviceClient(clients, pick)` is the one read of a client off a client set: it reports no client when the set or the field is nil. A guard fails on a call through a `ServiceClients` field that no function on the way to it checked, and another runs every related checker, Wave 2 enricher and detail enricher (child views' included, each given the detail-enrichment context the runtime builds) on every demo row with an empty client set and fails on a panic.
...
Wave 1 probes complete
  ...
  → EnrichmentCheckedMsg arrives
    → findings/FieldUpdates fold onto the type's cached rows (applyEnrichment + AmendRows).
      A result replaces the Wave-2 state of the rows it answered for. A row it
      reports as uninspected (`TruncatedIDs`) takes every finding the result
      did prove for it and keeps its other Wave-2 findings; every id keeps its
      state when the probe came back empty-handed with an error. The menu
      badge counts the folded rows, never the result map.
...
**Demo mode**: Wave 2 runs in demo mode as it does on a live account, against the typed fakes. `--no-cache` still runs Wave 2 (it only disables disk persistence, not capabilities).
...
- `EnrichmentTruncatedIDs map[string]map[string]string` — per-type record of the resource IDs the enricher could not inspect, each naming the check that did not answer: the refused API call (read off the SDK operation error by `checkOf`), the cause an unusable answer stated, or the inspection cap. The detail Attention entry renders it as `not inspected: DescribeInstanceStatus`, so the operator is told which check failed and not only that one did. It is the ONLY store for that fact; `Controller.listUninspectedIDs` reads it for both rendered surfaces (the list Status cell reads `not inspected`, the detail Attention block gains a `Not inspected` entry), so the two can never disagree. `capAtEnrichmentCap` is the one place a work list is trimmed to the cap, precisely so the dropped rows land here instead of rendering as inspected-and-healthy; it first drops the rows the check cannot apply to (an email address for DKIM, a Lambda log group for metric filters, an automated snapshot for public sharing), so the cap is spent only on rows it can answer for and an inapplicable row is never marked. Every per-row loop runs through `ForEachRow`, which marks a row whose check the Wave 2 deadline stopped before it started as `stopped at the Wave 2 deadline` and hands the context error back. An account-wide walk that a refused page ended (`walkAccountPages`) names that call on the rows it never reached; only a walk that ran out of page budget reads `stopped at the inspection cap`. Saved per row (`cache.Row.Uninspected`) by the sweep's own save and read back on replay, so the mark outlives a restart; a row left at the inspection cap gets its checks on demand when its detail opens (`KindEnrichRow`).
...
A failed per-item call is recorded once, as a class and a cause, never as a string. `FailedCall` (and `UnusableAnswer`, for an answer that carries no error — a nil struct, a page that says it is truncated and names no marker) is the one writer of a `Failure`: the item's id, the class `ErrClass` assigns, and the cause `CauseOf` reads off the error's own fields. `MarkSkipped` is the Wave 2 form of the same record, adding the row to `TruncatedIDs` so it renders `?`. No fetcher, checker or enricher formats a failure itself. The record also names the operation, read from the SDK operation error's own fields: one aggregate can cover several calls — a related walk resolves a load balancer's name and then reads its listeners — and the id alone does not say which of them refused. A denied call's cause already names the action, so it is not decorated twice. A row marked uninspected says why. An enricher that could not read a resource records the failed call through `MarkSkipped` — or `MarkUnusable`, when the service answered without the field the check needs and there is no error to read a cause from — and folds the records into its returned error. Writing `TruncatedIDs` directly is the silent-skip shape, where the row renders `?` and nobody can say what refused, so no enricher does: `markUninspected` is the only writer of the set and every recorder routes through it. The documented race (the resource went away between the list call and the per-item one) is `MarkSkipped`'s own contract, decided at that one site rather than re-spelled at each call site. `TestConformance_UninspectedRowsRecordTheirReason` accepts zero direct writes outside `issue_enrichment.go`. A per-item failure travelling out as the enricher's error is not an abort: the walk finishes, every other row keeps its findings, and `Truncated` is raised only where the pass can emit an issue, so an informational-only pass reports a coverage gap without lower-bounding the badge.

**A cap marks whichever fact it made uncertain.** `EnrichmentCap` bounds how many resources an enricher inspects at all, so the rows past it are marked uninspected and render `?` — nobody looked at them. `PerParentPageCap` bounds how far a per-parent walk enumerates INSIDE a resource that WAS inspected: the check completed and what is uncertain is only the number it produced, so its marking is the `+` on that count (`resource.FormatTruncated`), never a `TruncatedIDs` entry. That is why the two stay separate bounds rather than one helper family: a single marking would be a lie in whichever direction it went. A capped walk marked uninspected reports a completed check as one nobody finished, and a capped inspection marked with a `+` claims a check nobody ran.
...
`IsNotFoundErr` reads one table, `notFoundCodes`, and nearly every entry in it is a code alone: a code that means one thing carries the whole fact, and demanding a message from it would tie classification to wording AWS is free to change. Three codes do not mean one thing — autoscaling and CloudFormation both answer `ValidationError`, Athena answers `InvalidRequestException`, and each is also what the service returns for a request it could not parse. Those entries carry the service's own not-found sentence as well, because reading the code alone there turns a call a9s built wrong into a resource that went away and `MarkSkipped` swallows it: the row is marked uninspected and no failure is recorded, which is the one outcome the partial-answer machinery exists to prevent. The stored fragment is the sentence's subject and not its verb — `Stack with id`, not `does not exist` — since a validation failure borrows the verb while describing an argument.
...
**Generated view files carry a stamp.** `config.GeneratedViewsVersion` is written as a `generated: <n>` header into every file `EnsureViewsDir` produces, and bumped in the same change that adds, removes or renames a built-in column, or that changes where an existing one reads its value from. On launch, a file already at the current stamp is left byte for byte alone.

**An upgrade inserts only what the versions after the file's stamp introduced.** A built-in column the file does not carry is the operator's deletion — absence is the only record a view file keeps of that decision — so it stays absent. A migration that re-added every missing built-in would reverse the decision, and reverse it again on the next start, so deleting the column twice would not help either. `viewColumnAdditions` records, per stamp, the column the built-in views gained; that table is the whole difference between a column an installation has never been offered and one its operator deleted. A file with no stamp at all predates the table — the build that wrote it generated none of the current vocabulary — so every built-in is new to it.

Both writers ask that same question. A file whose every column is still exactly what its stamp's build generated (the current defaults, or the pre-correction column recorded in `viewColumnChanges`) has never been touched: it takes this build's column set and **order** wholesale, less the columns it deleted, which is how a corrected default order reaches an installation that already exists. Reordering columns and changing nothing else is indistinguishable from never having opened the file, so such a file is reordered back. A file carrying any other column is the operator's: it keeps every column and the order it has, and gains only the built-ins introduced after its stamp, each inserted at its built-in position.

Add a row to `viewColumnAdditions` in the same change that adds a column to a catalog literal's `Columns`, and bump the stamp with it. Forgetting is silent and lands on existing installations alone — a column with no entry reaches new installations and nobody else — so it has a gate rather than a convention: `TestDefaultColumnsAreTheBaselinePlusTheRecordedAdditions` seeds every view as the oldest migratable build wrote it, runs the real `EnsureViewsDir`, and fails naming the built-in column that never arrives.

Corrections reach the operator's own file too, **field by field**. A `viewColumnChanges` entry names a title and the whole column the previous build generated for it; on a column the file still carries, every field still holding that build's value takes this build's value, and a field the operator changed stays theirs. So a column they merely widened still picks up a corrected source, and a correction of a kind the table has never carried before needs no new machinery. The limit is what a comparison can see: a field an operator sets back to the value the previous build wrote reads as untouched, and the correction lands on them once. Without the stamp, an operator who ran a9s once keeps that day's column set forever.

**A corrected column source travels with the stamp.** A column whose source changes keeps its title, so the missing-title rule never sees it and an operator who had already launched a9s would keep reading the field the column was corrected away from — a cell that looks like it works and is wrong. `viewColumnChanges` (`core/config/ensure_views.go`) records, per stamp, which built-in column changed and the whole column the previous build generated for it. On upgrade, every field of an on-disk column still holding that previous value takes this build's; any other value under that title is the operator's own and is left exactly as it is. What the column is corrected TO is not stored in the table — the catalog is the one place a built-in column is defined. Add a row there in the same change that alters a built-in column's `Path`, `Key`, `Width` or `SortKey`.

One migration is recorded as a class rather than a row per column: at stamp 6 every built-in column took the key its type declares (`keyMoveVersion`), so a file written before it differs from this build's column in `Key` and in no other field, for 325 columns across 77 files. A row apiece would have said the same sentence 325 times, and a table nobody reads is a table the next correction is left out of.
...
Checkers never call these constructors. They report what they read as a `relatedRead` — the ids found, `partial` for a place read in part, `unread` for a place not read, the `failure` that says why — and `relatedAnswer` in `core/aws/related_fetch.go` picks the state: exact when every place was read over a whole list, a lower bound when something was read in part or a place could not be read beside ids found elsewhere, unknown when nothing was read, the error when a place's one call failed and nothing was found, and a proven zero only when everything was read and held nothing. A relation that lives in several places folds one read per place with `joinReads` (or `alsoRead` onto a result); a `PageAll` walk's `(complete, err)` becomes a read through `pagedRead`, so the items found before a failure stay a lower bound; a scan with one read per target row tallies them in `rowReads`. `foundNone`, `NotRead`, `keyMissing` (the row lacks the id it would ask with) and `ReadFailed` are shorthands. A read never sent — the session has no client for it, or the row carries no details yet — is decided by `noCallMade`, which `ReadFailed`, `unreadBy` and `rowReads.fail` all consult, so such a row is unknown on every path and never an error. A guard test fails on any constructor call outside that file, and another on a no-call error tested anywhere else.

**Matching** is done in one place per kind of join. Names join on their boundary (`core/aws/predicates.go`: `textNames`, `nameUnder`, `refNamesResource`, `imageRefersToRepo`, `sesRecipientMatches`), so `api` never matches `api-worker` and an ARN known for a row is compared whole. An EventBridge rule's pattern is read the way EventBridge reads it by `ebPatternMatches` (`core/aws/eb_pattern.go`), with every content filter EventBridge documents: a pattern that can match an event naming the resource is a match, one it cannot read — an undocumented operator, a filter of the wrong shape, a field the events do not carry — or one that matches every resource alike makes the count a lower bound.

**Call budget.** A checker that must make one AWS call per row of its target type passes the rows it calls for through `fanOut`, which keeps `EnrichmentCap` of them and reports the rest as a lower bound. Only the pivots `docs/related-resources.md` rule 7 names, each with the AWS citation that no reverse read answers it, may do so; a guard reads rule 7 and fails when a checker reaching `fanOut` is not named there, a named pivot's checker does not, or a loop makes an AWS call per row without going through `fanOut`. A target list a checker reads must have a fetcher, so no checker is handed a list that was never read.
...
**Call coalescing** (`core/aws/coalesce.go`): SFN/SNS/S3/ECS/Lambda transports are wrapped in singleflight decorators whose keys are namespaced by the operation ID (`WithDetailOp` on the context, applied once per lane — the generic engine for enrichers, `RunRelatedDef` for checkers). Within one operation, the enricher and every related checker share a single in-flight call per API key while it is still in flight (in-flight dedup, exactly as before); once that call completes, its result is retained in a small per-decorator, per-operation bounded LRU (`completedResultMemo`, 128 entries) so any LATER call for the same key under the SAME operation returns the memoized result instead of re-fetching — an SFN detail refresh performs one `DescribeStateMachine` in total even when the web drain runs the enricher and every related checker sequentially rather than concurrently. The memo is never shared across operations (keyed by operation ID; a refresh mints a new operation and therefore a new, empty namespace) and never applies to non-operation calls (`opID == 0`) — those always re-execute after the previous one finishes, exactly as before this memo existed. The shared call answers every caller that joined it, so it runs on a context none of them can cancel (`context.WithoutCancel`), bounded by its own timeout (`coalescedFlightTimeout`); a caller whose own context ends stops waiting and returns its context error while the flight finishes for the rest. Only an answer that holds for the whole operation is memoized: a success, plus S3's `NoSuchBucketPolicy` absence and, for ECS, a refusal AWS itself returned (`isAWSRefusal`). A throttle or a timeout is asked again by the next caller rather than replayed to it. Every decorator's method set carries each optional interface its callers assert on the client (a decorator that embeds only the base API would fail that assertion and silently take a check away); `coalescingECSContainerInstances` is the ECS variant for that reason.
...
**Fetch provenance** (`messages.FetchProvenance`) is what enforces that boundary. Every fetch lane — canonical list, related-filtered drill, `FetchByIDs`, child list — emits the same `messages.ResourcesLoaded`, so the message alone cannot say whether its rows are a canonical full replace or a scoped subset; `core/app`'s `handleResourcesLoadedEvent` — the one writer of that store for a delivery — admits a write only when `msg.Provenance.CanonicalList()`. The zero value is `FetchProvenanceUnknown` and `CanonicalList()` reports **false** for it, so a lane added later that forgets to declare provenance under-populates the store rather than corrupting it — the failure a test catches, not the one that reaches disk. `ProvenanceForContinuation` classifies `KindFetchMore` continuations from the parent context for both the executor and the TUI adapter; the two lanes share it because independently-written classifiers are how they drift apart.
...
Missing configs fall back to the built-in defaults: the detail fields from `core/config/defaults_*.go` (one file per service category) and the list columns from the type's catalog literal.
...
Wave 2 enrichment runs against the fakes as it does against AWS; the `isDemo` flag only keeps console links from being opened, since the demo resources exist in no account.

**Fixtures are copied for every reader.** Each fixture set is built once and every `New<Service>Fixtures()` call returns a deep copy of it (`shared` in `core/demo/fixtures/shared.go`), so a fake, a fetcher or a test that writes into what it was handed writes into its own copy. The `tests/unit` `TestMain` takes a copy of every set and of the exported fixture variables before the run (`fixtures.MutationCheckForTest`) and fails the binary when any of them differs afterwards: a write that bypassed the copy would otherwise change what every later test reads.
...
- `WithIsDemo(true)` — mark session as demo mode (console links are not opened; set by `--demo` CLI bootstrap). `--demo` supplies the fake clients and nothing else: whether the app keeps a disk cache is `--no-cache`'s answer there as everywhere else, so `./a9s --demo` runs the cache-load-then-sweep chain an installation runs, which is the path the menu's issue badges are written on. The demo smoke scripts point `A9S_CONFIG_FOLDER` at a temp directory of their own so a smoke neither reads nor leaves anything in the operator's cache.
...
3. **Add the detail fields** in `core/config/defaults_<category>.go` — the list columns are the literal's own `Columns` and need nothing here — then regenerate the on-disk views (`go run ./cmd/viewsgen/`).
...
**Every list screen has its own ordering counter.** The session-wide counters answer "is this result from the account and region I am looking at now"; they say nothing about which of two fetches for the same list was requested last. `Session.ListFetchSeq` closes that: every list fetch takes the next value for the screen that issued it at dispatch time (`Core.StampListFetchSeq`, called from the Controller's dispatch boundary and from the TUI's, exactly where the dispatch snapshot is stamped), carries it on the task and then on `messages.ResourcesLoaded.ListSeq`, and `Core.ListResultSuperseded` accepts only the newest value handed out. Ordering is one of two questions a result has to answer; the other is WHICH SCREEN asked. Two lists of one type — an AMI's EC2 drill stacked on another drill of the same type — share their type and their lane, so neither the sequence nor the provenance can tell them apart, and a continuation the deeper one is still waiting for lands on whichever is topmost. Every list screen takes a stable instance id when its state is created (`ListState.instance`, session-local and never serialised); every fetch it dispatches carries that id, and the result and the failure carry it back, so a page returns to the screen that asked for it or to nowhere at all. The same id guards the off-lock body build: two screens of one type have equal memo keys whenever their rows, filter and sort agree, so a body built for a screen that has since been popped would otherwise install into the one underneath. The question is asked ONCE per message, at the seam every lane routes a result through (`Core.HandleResourcesLoaded`); its answer, together with the canonical short name it keyed its work by, leaves as a `ListResultVerdict` and is stamped onto the message (`runtime.StampListResult`), so the screen state that applies the result reads what the seam decided instead of deciding again. A superseded message is still delivered to that screen state, which owes the discarded request the retirement of the activity flag it raised — nothing else will ever come to clear it. An on-entry verification overtaken by a Ctrl+R, a load-more or another refresh is discarded whole, whatever its rows look like. This replaced a pair of content heuristics, one on the screen and one in the row store, that guessed staleness from shape: they could recognise only a smaller, still-truncated subset of what was already there, so a superseded result of the same size or an exact one was applied, while a legitimate Ctrl+R reset to page 1 of an already-exact list was discarded. The enrichment-rerun reseed reads the same rule rather than ordering by its own counter: `EnrichmentTypeGen` says which rerun a result answers, never which of two results for the list is newer, and the two disagree whenever the later request is not itself a rerun (Ctrl+R, then `m` before the refresh lands). The counter is keyed by that screen instance and not by the type, which is what lets every lane draw one: a drill's load-more shares the type with the list beneath it, and a shared counter let each supersede the other's requests. A task no list screen issued draws none — there is nothing for it to be ordered against — and an unstamped result (a cache-seed replay) carries no ordering claim and is applied as-is. A screen's counter dies with the screen: an instance is never reused, so once the screen leaves the stack its entry answers nothing and is dropped, which is also what keeps the map from growing by one entry per drill for the life of the session. A failure is the other outcome of the same request and is built beside its success by one seam (`runtime.FetchOutcome.Msg`, the only construction of `messages.APIError` in the tree), so it can never carry a different screen, sequence or lane from the success it replaces — the two used to be hand-built literals side by side, and several sites had drifted. A result that names no screen was dispatched by no screen, so nothing is waiting on it and it reaches none: the by-type scan that used to route those is gone, because two lists of one type share their type and their lane, and returning the topmost of the two was a guess.
