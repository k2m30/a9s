...
**Service clients** (`core/aws/client.go`): a session may hold a client set without a given service's client, so a `ServiceClients` field is nil until checked. `serviceClient(clients, pick)` is the one read of a client off a client set: it reports no client when the set or the field is nil. A guard fails on a call through a `ServiceClients` field that no function on the way to it checked, and another runs every related checker, Wave 2 enricher and detail enricher on every demo row with an empty client set and fails on a panic.
