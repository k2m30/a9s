...
type ResourceTypeDef struct {
    ...
    Columns     []domain.Column  // table columns for list view — Key/Path (what the cell reads), Title, Width, SortKey, Sortable
...
`Columns []domain.Column` is the ONE declaration of a type's list columns: which there are, in what order, each with its title, its width, where its cell reads its value (`Key`, `Path`), what it sorts on (`SortKey`) and whether it sorts at all (`Sortable`). `config.DefaultConfig()` derives the built-in view's `list:` block from it — `catalogListColumns` in `core/config/defaults.go` — so there is no per-type column list in `core/config` to drift from it, and `go run ./cmd/viewsgen/` writes what the catalog says into `.a9s/views/*.yaml`.

It was two declarations, and they disagreed. The defaults declared columns the catalog did not (a Status column on twenty-nine types), at different widths, in a different order, and the catalog declared columns no view ever rendered (six on `ct-events`); which of the two a change reached depended on which file its author happened to open, which is how lambda's Handler column existed on the demo bench and on nobody's account. `Sortable` stays the catalog's alone: it is a fact about the type, and a view file has no field for it.

`resource.ResolveListColumnCascade` is still the one resolver, because an operator's own file is a third thing: it owns which columns there are, their order and their widths, and the catalog's `Key` is merged onto every title both declare, through `mergeListColumn`. A type no view declares at all resolves to the catalog's columns alone.
