...
**The cursor keeps its entry, not its index.** An enrichment result rebuilds the Attention block under whatever the operator is reading, and a finding that outranks the ones already there sorts above them, so every entry below it moves down — sometimes at a block length the rebuild happens to leave unchanged. `applyFindingToState` (`core/app/detail_state.go`) therefore names the entry the cursor is on before the rebuild and finds that name again after it, landing on the section header only when the entry is gone. A cursor below the block still moves by the change in the block's size. The terminal host reads the same `FieldCursor` off the controller's detail body, so both surfaces behave identically.
