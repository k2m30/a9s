...
An error becomes words in exactly one place: `aws.ErrClass` assigns the class (`classifyProbeErr` delegates to it, so the menu row's cause word and a rendered failure can never drift apart) and `aws.CauseOf` reduces the error to what an operator can act on — the SDK's `operation error <Service>: <Op>` preamble and the per-call request id, host id and encoded message removed, whatever the error class. Every flash and error-history line that names a failure phrases it through `CauseOf`, never through an error's `%v`.
