...
`Columns []domain.Column` is the source for what a list cell reads (`Key`). The built-in view defaults (`core/config/defaults_*.go`) declare the same columns a second time, because a view file on disk is what an operator edits: the defaults may add columns the catalog does not declare, set the widths an operator will adjust, and carry the rendering hints the catalog literal has no field for (`Path`, `SortKey`).

`resource.ResolveListColumnCascade` resolves the pair on one path, whatever the caller holds. The VIEW — the operator's file when one is loaded, the built-in default otherwise, which is the fallback `config.GetViewDef` already performs — owns which columns there are, their order and their widths. The CATALOG owns what each cell reads: its `Key` is merged onto every title both declare, through `mergeListColumn`. A type no view declares at all, which is how child types arrive, resolves to the catalog's columns alone.
