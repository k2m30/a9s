...
- **Per-row cause.** A probe that hard-fails emits `PatchMenuProbeCause` carrying the class `classifyProbeErr` already assigned (`access-denied`, `expired`, `throttled`, `timeout`, `transport`, or a raw AWS code); the row keeps its cached count and shows that class's own word (`denied`, `expired`, `throttled`, `timeout`, `transport`, or `error` for a code the table does not name) in the alias column. A partial result is not a refusal and clears the mark, as does any successful probe. The error text is classified once, in `classifyProbeErr`, and never parsed again.
