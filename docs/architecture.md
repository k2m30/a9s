...
**Generated view files carry a stamp.** `config.GeneratedViewsVersion` is written as a `generated: <n>` header into every file `EnsureViewsDir` produces, and bumped in the same change that adds, removes or renames a built-in column, or that changes where an existing one reads its value from. On launch, a file already at the current stamp is left byte for byte alone. A file with an older stamp (or none) is read for evidence that the operator has edited it: a file whose every column is still the source and width its stamp's build generated (the current defaults, or the pre-correction values in `viewColumnSourceChanges`) has never been touched, and takes this build's column set and **order** wholesale, which is how a corrected default order reaches an installation that already exists. A file carrying any other column, source or width is the operator's: it keeps every column and the order it has, and only gains the built-in columns whose titles it does not carry, each inserted at its built-in position. Reordering columns and changing nothing else is indistinguishable from never having opened the file, so such a file is reordered back. Without the stamp, an operator who ran a9s once keeps that day's column set forever.
