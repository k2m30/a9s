...
An operator's own file is where the two used to be told apart by accident: a column was the status column if its title said Status, or if its key said `status` whatever the type calls its own key, so a renamed column with a hand-typed `key: status` worked. Neither is true now, so the migration owns that key like any other it moved: at stamp 7 a column carrying the literal `status` on a type that calls it something else takes the type's key, matched on the key alone because a renamed column is exactly the one the title merge cannot reach. A key that names nothing on the type at all is a cell that is blank on every row, so `config.LoadFromDirs` reports it — naming the file and the key, once — and hands back the config anyway. Both lanes keep the file and show the report in the same words: `config.ReportText` composes the one sentence — which says whether the operator's file or the built-in defaults are in use, because a parse error means nothing loaded while an unfillable key costs them one column — and the TUI flashes it, the web page carries it in the flash its own header renders, and the web server logs a copy to stderr. The web used to write it to stderr alone, after replacing the whole config with the defaults.
