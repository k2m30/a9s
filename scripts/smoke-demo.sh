...
if [ "$ready" -ne 1 ]; then
	echo "smoke: app did not reach a populated menu within 30s; capture in $CAPDIR/menu.txt" >&2
	exit 1
fi
...
expect ng.txt "health issue" "ng pool with a health issue says so"
expect ng.txt "degraded" "ng degraded pool says so"
