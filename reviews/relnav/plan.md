...
1. **One ref resolver for every emitted id and navigable field** (ID, NAV). The remaining hand
...
2. **One coverage rule for a pivot's count** (CARD). `anyDegraded` stops meaning "the list is a
...
3. **One matcher per kind of join** (MATCH). One EventBridge pattern evaluator for the ecr, s3 and
...
4. **Region carried by the source** (CALL). Global-service pivots (cf → lambda@edge), a bare name
...
5. **Checkers read every source the contract names** (CONTRACT, other). Per-type fixes on the
...
6. **Per-parent reads paged** (PAGE). kinesis tags, IAM group sweep, ec2 by-id batching.
