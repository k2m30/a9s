...
- A row a9s could not inspect now says so. Its Status cell reads `not inspected` instead of a lifecycle word, and its detail view carries an Attention entry naming the check that did not answer. Its colour is unchanged: an unknown posture is not an issue, it is just not a clean bill of health.
