...
const (
	...
	ACMAcmeIssued = "renewal-window.acme-corp.com"
)
...
var sharedACMFixtures = shared(func() *ACMFixtures {
	return &ACMFixtures{
		Certificates: []acmtypes.CertificateSummary{
			{
			...
			// regardless of when the demo runs. ACMAcmeIssued: the one
			// certificate issued over ACME, which ListCertificates answers only
			// when CertificateKeyPairOrigins names ACME.
			{
				DomainName:               aws.String(ACMAcmeIssued),
