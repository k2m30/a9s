...
func (c *Core) DeleteEnrichmentTruncatedIDs(rt string) {
	c.session.EnrichmentRowsForget(rt)
}
...
// renders its rows without a fetch, identically on both renderers. It is also
// the set a detail's navigable field resolves against: the rows the related
// checkers see, so a field and the related row read a reference alike.
