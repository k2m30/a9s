...
var messagingTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Columns: []domain.Column{
			{Key: "queue_name", Title: "Queue Name", Width: 36},
			{Key: "state", Title: "Status", Width: 12},
			{Key: "approx_messages", Title: "Messages", Width: 10},
			{Key: "approx_not_visible", Title: "In Flight", Width: 10},
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "lambda", DisplayName: "Lambda Functions", Checker: checkSQSLambda, NeedsTargetCache: false, Truncated: true, Mirror: true},
			{TargetType: "sqs", DisplayName: "Dead Letter Queues", Checker: checkSQSSQS, NeedsTargetCache: true, Truncated: true},
			{TargetType: "sns-sub", DisplayName: "SNS Subscriptions", Checker: checkSQSSNSSub, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "sns", DisplayName: "SNS Topics", Checker: checkSQSSNS, NeedsTargetCache: true, Truncated: true},
			{TargetType: "eb-rule", DisplayName: "EventBridge Rules", Checker: checkSQSEbRule, NeedsTargetCache: true, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "display_name", Title: "Topic Name", Width: 40},
			{Key: "state", Title: "Status", Width: 12},
			{Key: "subs_count", Title: "Subs", Width: 6},
			{Key: "topic_arn", Title: "Topic ARN", Path: "TopicArn", Width: 60},
		},
		Children: []domain.ChildViewDef{{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "sns-sub", DisplayName: "Subscriptions", Checker: checkSNSSub, NeedsTargetCache: true, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "topic_arn", Title: "Topic ARN", Path: "TopicArn", Width: 48},
			{Key: "state", Title: "Status", Width: 12},
		...
		Related: []domain.RelatedDef{
			{TargetType: "sns", DisplayName: "SNS Topic", Checker: checkSNSSubTopic, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "lambda", DisplayName: "Lambda Function", Checker: checkSNSSubLambda, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "sqs", DisplayName: "SQS Queue", Checker: checkSNSSubSQS, NeedsTargetCache: true, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "environment_name", Title: "Environment", Path: "EnvironmentName", Width: 28},
			{Key: "application_name", Title: "Application", Path: "ApplicationName", Width: 24},
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "tg", DisplayName: "Target Groups", Checker: checkEbTG, NeedsTargetCache: false, Truncated: true},
			{TargetType: "sg", DisplayName: "Security Groups", Checker: checkEbSG, NeedsTargetCache: false},
			{TargetType: "role", DisplayName: "IAM Role", Checker: checkEbRole, NeedsTargetCache: false, Truncated: true},
			{TargetType: "s3", DisplayName: "S3 Buckets", Checker: checkEbS3, NeedsTargetCache: false, Truncated: true},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "kinesis", DisplayName: "Kinesis (targets)", Checker: checkEbRuleKinesis, Truncated: true},
			{TargetType: "lambda", DisplayName: "Lambda (targets)", Checker: checkEbRuleLambda, Truncated: true, Mirror: true},
			{TargetType: "logs", DisplayName: "Log Groups (targets)", Checker: checkEbRuleLogs, Truncated: true},
			{TargetType: "sfn", DisplayName: "Step Functions (targets)", Checker: checkEbRuleSFN, Truncated: true, Mirror: true},
			{TargetType: "sns", DisplayName: "SNS (targets)", Checker: checkEbRuleSNS},
			{TargetType: "sqs", DisplayName: "SQS (targets)", Checker: checkEbRuleSQS, Mirror: true},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "stream_name", Title: "Stream Name", Path: "StreamName", Width: 36},
			{Key: "status", Title: "Status", Path: "StreamStatus", Width: 12},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeKinesisUpdating, Phrase: "updating", Severity: domain.SevWarn, Source: "wave1", Detail: "A change to the stream is being applied. Only a resharding operation changes the shard list; a retention, encryption or capacity-mode change leaves it alone. Check which operation is running, and refresh shard discovery in any hand-written consumer if it is a reshard."},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "cluster_name", Title: "Cluster Name", Path: "ClusterName", Width: 28},
			{Key: "cluster_type", Title: "Type", Path: "ClusterType", Width: 14},
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "lambda", DisplayName: "Lambda Functions", Checker: checkMSKLambda, NeedsTargetCache: true, Truncated: true, Mirror: true},
			...
			{TargetType: "secrets", DisplayName: "Secrets Manager", Checker: checkMSKSecrets, Truncated: true},
	...
	},
	{
		...
		Columns: []domain.Column{
			...
			{Key: "type", Title: "Type", Path: "Type", Width: 10},
			{Key: "state", Title: "Status", Width: 12},
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "logs", DisplayName: "Log Groups", Checker: checkSFNLogs, NeedsTargetCache: false},
			{TargetType: "role", DisplayName: "IAM Role", Checker: checkSFNRole, NeedsTargetCache: false},
			{TargetType: "eb-rule", DisplayName: "EventBridge Rules", Checker: checkSFNEbRule, NeedsTargetCache: true, Truncated: true, Mirror: true},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: sfnCodeLoggingOff, Phrase: "execution logging off", Severity: domain.SevWarn, Source: "wave2", Detail: "Execution logging to CloudWatch is off. A standard workflow still keeps its execution history in Step Functions for ninety days, so a recent failure can be read there; an express workflow keeps nothing at all without logging. Turn logging on for express workflows, and for any workflow whose history you need beyond ninety days or alongside your other logs."},
	...
	},
	{
		...
		Columns: []domain.Column{
			{Key: "identity_name", Title: "Identity", Path: "IdentityName", Width: 36},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeSESSendingDisabled, Phrase: "sending disabled", Severity: domain.SevWarn, Source: "wave1", Detail: "The service reports that this identity cannot send, most often because its verification or authorisation is incomplete rather than because a switch was thrown. Resolve the identity's own status first, then check the account-level sending status and any configuration set the sender uses, which are the two places sending is actually turned off."},
