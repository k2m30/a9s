...
// many IDs were requested (up to the AWS per-request limit). Missing IDs are
// handled by fetchEC2ByIDs.
...
func fetchEC2ByIDs(ctx context.Context, op, notFoundCode, idPrefix string, ids []string, describe func([]string) ([]resource.Resource, error)) ([]resource.Resource, error) {
	...
	remaining, missing := requested, []string(nil)
	for {
		rows, err := describe(remaining)
		if err == nil {
			for _, id := range remaining {
				if !slices.ContainsFunc(rows, func(r resource.Resource) bool { return r.ID == id }) {
		...
		if len(remaining) == 0 {
			return nil, AggregateMissing(op, missing, len(requested))
		}
	}
}
