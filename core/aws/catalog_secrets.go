...
var secretsTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeKMSStateUnavailable, Phrase: "<key state>", Severity: domain.SevBroken, Source: "wave1", Detail: "The key is not usable in its current state, so every operation depending on it fails; for an external or custom key store this usually means the backing store is disconnected. Reconnect the key store, or restore the key material."},
