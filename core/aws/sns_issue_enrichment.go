...
func snsTopicRow(ctx context.Context, clients *ServiceClients, r resource.Resource, ownAccount string) (IssueEnricherResult, []Failure) {
	...
	snsTopicPosture(ctx, clients, &row, &rowFailures, r.ID, ownAccount)
	if pageCapped {
	...
	if len(subs) == 0 {
		setWave2Finding(&row, r.ID, snsCodeNoSubscribers, nil)
		return row, rowFailures
	}
	allPending := true
	...
	if allPending {
		setWave2Finding(&row, r.ID, snsCodeAllPending, nil)
	}
	return row, rowFailures
}

// snsTopicPosture reads the topic's own attributes — the access policy and
// the encryption key — and records what they expose, into the calling
// worker's own result.
