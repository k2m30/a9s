...
var databasesTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDBIIncompatibleOptionGroup, Phrase: "incompatible-option-group", Severity: domain.SevBroken, Source: "wave1", Detail: "The option group attached to this instance does not work with its engine version, so it cannot come up. Move the instance to an option group built for the version it runs, or roll the version back."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDBCIncompatibleParameters, Phrase: "parameter group incompatible", Severity: domain.SevBroken, Source: "wave1", Detail: "The cluster parameter group holds a setting the engine rejects, so the cluster will not come up with it applied. Correct the parameter and reboot the cluster; the events list names the setting."},
			...
			{Code: CodeDBCDeletionProtectionOff, Phrase: "delete-protection off", Severity: domain.SevWarn, Source: "wave1", Detail: "One delete call removes this cluster and every instance in it. Turn deletion protection on so it has to be disabled deliberately first."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			{Code: CodeRedshiftIncompatibleHSM, Phrase: "broken: incompatible-hsm", Severity: domain.SevBroken, Source: "wave1", Detail: "The cluster cannot reach the hardware security module holding its encryption key, so it will not come up. Check the client certificate for that module and the network path to it, then restore the connection."},
			{Code: CodeRedshiftIncompatibleNetwork, Phrase: "broken: incompatible-network", Severity: domain.SevBroken, Source: "wave1", Detail: "The cluster's subnet group no longer provides what it needs — free addresses, or the Availability Zone it was created in — so it cannot start. Fix the subnet group, then restore the cluster."},
			{Code: CodeRedshiftIncompatibleParameters, Phrase: "broken: incompatible-parameters", Severity: domain.SevBroken, Source: "wave1", Detail: "A value in this cluster's parameter group is rejected, so the cluster will not come up with it applied. Correct the parameter group and reboot the cluster."},
			{Code: CodeRedshiftIncompatibleRestore, Phrase: "broken: incompatible-restore", Severity: domain.SevBroken, Source: "wave1", Detail: "The restore from snapshot failed, so this cluster holds no usable data. Check the snapshot's node type and encryption against the target, then restore again."},
			{Code: CodeRedshiftHardwareFailure, Phrase: "broken: hardware-failure", Severity: domain.SevBroken, Source: "wave1", Detail: "A node's underlying hardware failed. Redshift replaces the node itself, but the cluster is degraded or unavailable until it does; watch the events and restore from the latest snapshot if it does not recover."},
			{Code: CodeRedshiftStorageFull, Phrase: "broken: storage-full", Severity: domain.SevBroken, Source: "wave1", Detail: "The cluster has no disk left, so queries that need to spill fail and loads are rejected. Delete or unload cold tables, vacuum to reclaim space, then resize to more storage."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDBISnapUnencrypted, Phrase: "unencrypted", Severity: domain.SevWarn, Source: "wave1", Detail: "The snapshot's contents are stored unencrypted, and anything restored from it starts unencrypted too. Copy it with a KMS key, restore from the encrypted copy, then delete this one."},
			{Code: dbiSnapOrphanCode, Phrase: "orphan: source DB deleted", Severity: domain.SevBroken, Source: "wave2", Detail: "The instance this snapshot was taken from no longer exists, so it is the only copy of that data and nothing is producing newer ones. Keep it deliberately and record why, or delete it — either way its storage is billed every month."},
			{Code: dbiSnapPastRetentionCode, Phrase: "automated, <N>d past retention", Severity: domain.SevBroken, Source: "wave2", Detail: "This automated snapshot has outlived the instance's retention period, which means something other than the backup policy is keeping it and nobody is managing its lifecycle. Confirm it is still needed, then either convert it to a manual snapshot with an owner or delete it."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDBCSnapUnencrypted, Phrase: "unencrypted", Severity: domain.SevWarn, Source: "wave1", Detail: "The snapshot is stored unencrypted, and any cluster restored from it inherits that. Copy it with a KMS key, restore from the copy, then delete the unencrypted original."},
			{Code: dbcSnapOrphanCode, Phrase: "orphan: source cluster deleted", Severity: domain.SevBroken, Source: "wave2", Detail: "The cluster this snapshot came from is gone, so it is the last copy of that data and nothing newer is being produced. Give it an owner and a retention decision, or delete it."},
