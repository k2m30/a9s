...
var databasesTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "kms", DisplayName: "KMS Key", Checker: checkDbiKMS, Mirror: true},
			...
			{TargetType: "dbi-snap", DisplayName: "DB Instance Snapshots", Checker: checkDbiDBISnap, NeedsTargetCache: true, Truncated: true, Mirror: true},
			...
			{TargetType: "secrets", DisplayName: "Secrets Manager", Checker: checkDbiSecrets, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "dbc", DisplayName: "RDS Clusters", Checker: checkDbiDBC, Mirror: true},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDBINotInBackupPlan, Phrase: "not covered by a backup plan", Severity: domain.SevWarn, Source: "wave2", Detail: "No backup plan backs up this database, so its retention is whatever the instance's own automated backups happen to be. An instance in a cluster (Aurora, DocumentDB, Neptune or a Multi-AZ DB cluster) is backed up only through that cluster: add the cluster to a plan, and for Aurora, DocumentDB or Neptune keep the engine type opted in under this Region's AWS Backup service opt-in. Any other instance: add it by its ARN or by the DB instance resource type; a plan that takes it in only by tag, by service name or by selecting everything backs it up only once its type is opted in there."},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			{TargetType: "trail", DisplayName: "CloudTrail Trails", Checker: checkS3Trail, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "cf", DisplayName: "CloudFront", Checker: checkS3CF, NeedsTargetCache: true, Truncated: true, Distinct: "distributions whose Origins.Items address this bucket"},
			{TargetType: "lambda", DisplayName: "Lambda (notifications)", Checker: checkS3Lambda, Mirror: true},
			...
			{TargetType: "cfn", DisplayName: "CloudFormation", Checker: checkS3CFN, Truncated: true, Distinct: "the stack this bucket's aws:cloudformation:stack-name tag names"},
			...
			{TargetType: "athena", DisplayName: "Athena WorkGroups", Checker: checkS3Athena, Truncated: true, Mirror: true},
			{TargetType: "glue", DisplayName: "Glue Jobs", Checker: checkS3Glue, Truncated: true, Mirror: true},
			{TargetType: "backup", DisplayName: "Backup Plans", Checker: checkS3Backup, Truncated: true},
			{TargetType: "eb-rule", DisplayName: "EventBridge Rules", Checker: checkS3EBRule, Truncated: true},
			{TargetType: "r53", DisplayName: "Route 53", Checker: checkS3R53, Truncated: true, Mirror: true},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: s3CodePublic, Phrase: "publicly accessible", Severity: domain.SevBroken, Source: "wave2", Detail: "This bucket is open to the internet — either AWS reports its policy as public, or its access control list grants a permission to one of the two public groups: anyone at all, including callers with no AWS account, or every signed-in AWS account. How far that reaches is the permission the row names, from listing the bucket to reading its objects or its access control list. The account-level public access block is not read here, so one set there may already be blocking this. Remove the wildcard-principal statements from the bucket policy and the public grants from the access control list, or turn on all four public access block settings."},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "dbi", DisplayName: "RDS Instances", Checker: checkDbcDBI, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "dbc-snap", DisplayName: "DB Cluster Snapshots", Checker: checkDbcDbcSnap, NeedsTargetCache: true, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeDDBNotInBackupPlan, Phrase: "not covered by a backup plan", Severity: domain.SevWarn, Source: "wave2", Detail: "No backup plan backs up this table, so nothing is scheduled to copy it and point-in-time recovery alone will not survive the table being deleted. Add it to a plan by its ARN or by the DynamoDB table resource type; a plan that takes it in only by tag, by service name or by selecting everything backs it up only once DynamoDB is opted in under this Region's AWS Backup service opt-in."},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "lambda", DisplayName: "Lambda Functions", Checker: checkEFSLambda, NeedsTargetCache: false, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			{TargetType: "dbi", DisplayName: "DB Instances", Checker: checkDBISnapDBI, NeedsTargetCache: true, Truncated: true, Mirror: true},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			{TargetType: "dbc", DisplayName: "DB Cluster", Checker: checkDbcSnapDBC, NeedsTargetCache: true, Truncated: true, Mirror: true},
		...
		Navigable: []domain.NavigableField{
			{FieldPath: "DBClusterIdentifier", TargetType: "dbc", Resolve: dbcSnapParentRow},
			{FieldPath: "VpcId", TargetType: "vpc"},
			{FieldPath: "KmsKeyId", TargetType: "kms"},
		},
		Findings: []catalog.FindingDef{
