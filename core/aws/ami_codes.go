...
const (
	...
	// (not launchable). Severity: SevDim.
	CodeAMIStateDim domain.FindingCode = "ami.state.dim"

	// CodeAMIDeprecated — AMI's DeprecationTime has passed; the AWS Console does
	// not recommend it for new launches. Severity: SevWarn.
