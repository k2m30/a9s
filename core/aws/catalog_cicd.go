...
var cicdTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "s3", DisplayName: "S3 (stack resources)", Checker: checkCfnS3, Truncated: true, Distinct: "the buckets this stack manages, from ListStackResources"},
			{TargetType: "eb-rule", DisplayName: "EventBridge Rules", Checker: checkCfnEBRule, Truncated: true},
		...
		Findings: []catalog.FindingDef{
			{Code: CodeCFNStackFailed, Phrase: "<failure status, in words>", Severity: domain.SevBroken, Source: "wave1", Detail: "The stack's last operation failed and the stack is now in a state that decides what you may do next. Read the exact status and the failure options the operation ran under, then choose between retrying, continuing the rollback, updating with a corrected template, or deleting; a stack left in a failed rollback needs the rollback continued before anything else is accepted."},
			{Code: CodeCFNStackRollback, Phrase: "<rollback status, in words>", Severity: domain.SevBroken, Source: "wave1", Detail: "CloudFormation attempted to undo the last change. A completed rollback means the previous template is what is deployed and the update delivered nothing; a rollback that itself failed means the stack is stuck part-way and needs the rollback continued before it will accept another operation. Read the events for the resource that started it either way."},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			{TargetType: "cb", DisplayName: "CodeBuild Projects", Checker: checkPipelineCB, NeedsTargetCache: false, Mirror: true},
			...
			{TargetType: "eb-rule", DisplayName: "EventBridge Rules", Checker: checkPipelineEbRule, NeedsTargetCache: false, Truncated: true},
			{TargetType: "ecr", DisplayName: "ECR Repositories", Checker: checkPipelineECR, NeedsTargetCache: false, Mirror: true},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			...
			{TargetType: "pipeline", DisplayName: "CodePipelines", Checker: checkCbPipeline, NeedsTargetCache: true, Truncated: true, Mirror: true},
			...
			{TargetType: "ecr", DisplayName: "ECR Repositories", Checker: checkCbECR, NeedsTargetCache: true, Truncated: true, Mirror: true},
			{TargetType: "s3", DisplayName: "S3 Buckets", Checker: checkCbS3, NeedsTargetCache: true, Truncated: true},
			{TargetType: "secrets", DisplayName: "Secrets Manager", Checker: checkCbSecrets, NeedsTargetCache: false, Mirror: true},
	...
	},
	{
		...
		Related: []domain.RelatedDef{
			{TargetType: "lambda", DisplayName: "Lambda Functions", Checker: checkECRLambda, NeedsTargetCache: true, Truncated: true, Mirror: true},
			...
			{TargetType: "pipeline", DisplayName: "CodePipelines", Checker: checkECRPipeline, NeedsTargetCache: true, Truncated: true, Mirror: true},
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeECRScanOnPushOff, Phrase: "scan on push off", Severity: domain.SevWarn, Source: "wave1", Detail: "This repository does not scan an image as it arrives, so a known vulnerability in a base layer can reach production without anyone being told. Check the registry's scanning configuration first, because enhanced scanning is set there and covers repositories continuously; if the registry uses basic scanning, turn scan on push on for this repository."},
...
var cicdChildTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Findings: []catalog.FindingDef{
			{Code: CodeCBBuildFailed, Phrase: "failed", Severity: domain.SevBroken, Source: "wave1", Detail: "The build did not succeed, so nothing it produced has been proven. Artifacts may still have been uploaded, because the upload phase runs even after the build phase fails, and a stale or partial artifact in the usual place is worse than none. Open the failing phase in the logs, and treat any output as unpublished until it has been rebuilt."},
