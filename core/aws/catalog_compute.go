...
var computeTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeEC2StateShuttingDown, Phrase: "shutting down", Severity: domain.SevWarn, Source: "wave1", Detail: "The instance is being terminated and will disappear shortly, taking anything on its instance-store volumes with it. If this was not intended, stop whatever issued the terminate call now — a terminated instance cannot be recovered."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeLambdaDeprecatedRuntime, Phrase: "runtime is end-of-life", Severity: domain.SevBroken, Source: "wave1", Detail: "This function runs on a runtime AWS no longer patches, so language and base-image security fixes will never reach it, and AWS eventually blocks updates and then invocations. Move the function to a supported runtime version and redeploy."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			{Code: CodeASGStateDeleting, Phrase: "delete in progress", Severity: domain.SevWarn, Source: "wave1", Detail: "The group is being removed and is terminating its instances, so the capacity it provided is going away. If anything still depends on it, stop the deletion now — once it completes, the group and its scaling history are gone."},
	...
	},
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeEBSStateDeleting, Phrase: "deleting", Severity: domain.SevWarn, Source: "wave1", Detail: "The volume is being deleted; its data is going with it and nothing else about it is worth reporting until it is gone."},
