...
var dataChildTypes = []catalog.ResourceTypeDef{ //nolint:gochecknoglobals // static catalog: intentional package-level var
	{
		...
		Findings: []catalog.FindingDef{
			...
			{Code: CodeGlueRunExpired, Phrase: "expired", Severity: domain.SevBroken, Source: "wave1", Detail: "The run exceeded the maximum time Glue allows and was ended, so it produced nothing usable. Reduce the work per run by narrowing the partition or raising the worker count, or restructure the job to process in batches."},
			...
			{Code: CodeGlueRunWaiting, Phrase: "waiting", Severity: domain.SevWarn, Source: "wave1", Detail: "The run is queued behind the job's concurrency limit and is not consuming workers yet. Raise the job's maximum concurrent runs, or stagger the triggers, if this queueing is delaying the pipeline."},
