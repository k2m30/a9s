...
type ColumnDef struct {
	...
	// SortKey is the view definition's optional sort override: the Fields key
	// to compare when the display value sorts wrongly. Carried on the resolved
	// column so the sort comparator reads the same column set the cells are
	// extracted from. Not serialized: sorting is decided controller side and
	// renderers receive rows already ordered.
	SortKey string `json:"-"`
